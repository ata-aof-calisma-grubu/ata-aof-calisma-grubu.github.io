...
1. Ölçme ile ilgili aşağıdaki bilgilerden hangisi <u>doğru değildir</u>?
    - A-) Ölçme, herhangi bir niteliğin belirli kurallara göre sayısal olarak ifade edilme sürecidir.
    - B-) Ölçme işlemi, sayarak, sınıflandırarak, derecelendirerek yapılabilir.
    - **Cevap C-) Ölçme işleminin amacı ölçüte göre uygun bir karara varmaktır.**
    - D-) Ölçme işlemi bir nitelik ifade etme yöntemi olduğu için objektif olmalıdır.
    - E-) Psikometri, psikolojik ölçme ve değerlendirme bilimidir.
...
2. Psikometrik açıdan “ölçme” kavramı neyi ifade eder?
    ...
    - D-) Elde edilen verilerin belirli ölçütlere göre yorumlanmasıdır
    - E-) Doğrudan iletişim yoluyla bilgi alışverişinde bulunma sürecidir.
...
3. Psikometride ölçme işlemi hakkında yukarıdaki ifadelerden hangileri doğrudur?
I. Ölçme herhangi bir niteliğin belirli kurallara göre sayısal olarak ifade edilme sürecidir
...
4. Aşağıdakilerden hangisi dolaylı ölçme işlemi ile ölçülebilir?
    - A-) Sertlik
    - **Cevap B-) Mizaç**
    - C-) Ağırlık
    - D-) Boy
...
5. Aşağıdakilerden hangisi dolaylı ölçme işlemiyle ölçülebilir?
    - A-) Boy
    - B-) Sıcaklık
    - C-) Tepki zamanı
    - **Cevap D-) Zeka**
...
### 1.3. Psikometrik Yaklaşım

6. Ölçme ve değerlendirmede psikometrik yaklaşımın temel amacı nedir?
    - **Cevap A-) Değerlendirme sürecini nesnel, standartlaştırılmış ve ölçülebilir hale getirmek.**
    - B-) Değerlendirme sürecinde, uzman yorumunun etkisinin maksimuma çıkarılması.
    - C-) Ölçüm hatalarının tamamen ortadan kaldırılması.
    - D-) Bireylerin psikolojik durumlarınna göre belirli kategorilere ayrılması.
    - E-) Bireylerin gelecekteki performanslarını tahmin edilmesi.

    > **Açıklama:** Psikometrik yaklaşımın temel amacı, değerlendirme sürecini nesnel, standartlaştırılmış ve ölçülebilir hale getirmektir. Bu yaklaşımda uzman yorumunun etkisi minimuma indirgenir.

    <hr />

### 1.4. Ölçek Türleri

7. Aşağıdaki ölçek tiplerinden hangisi hesaplama ve istatistiklerde kullanım için en uygundur?
...
### 1.4.2. Eşit Oranlı (Ratio) Ölçekler

8. Aşağıdaki ölçeklerden hangisi üzerinde tüm matematiksel işlemlerin yapılabildiği, en gelişmiş ölçektir?
    - A-) Nominal (Sınıflama)
    - B-) Ordinal (Sıralama)
    - C-) Interval (Eşit aralıklı)
    - **Cevap D-) Ratio (Eşit oranlı)**
    - E-) Likert tipi ölçek
...
9. Aşağıdakilerden hangisi eşit oranlı ölçeklere örnektir?
...
10. Aşağıdakilerden hangisi eşit oranlı ölçeklere örnek olarak verilebilir?
    ...
    > **Açıklama:** Ağırlık ve boy gibi fiziksel ölçüler, gerçek (mutlak) sıfır noktasına sahip olduğu için eşit oranlı ölçeklere örnektir.

    <hr />

### 1.4. Ölçek Türleri

11. Aşağıdakilerden hangisi yapısal niteliklerine göre ölçek çeşitlerinden <u>değildir</u>?
...
12. Aşağıdakilerden hangisi sınıflama ölçeklerine örnek olarak verilebilir?
    - A-) Sınav puanları
...
13. Bireyleri ya da nesneleri belirli kategorilere ayırarak tanımlayan; örneğin bir öğrenci grubunu saç rengine göre sarışın, kumral ve esmer olarak sınıflandıran ölçek türü aşağıdakilerden hangisidir?
...
14. Aşağıdakilerden hangisi sıralama ölçeklerine örnek olarak verilebilir?
    - A-) Hava sıcaklıkları
...
15. Aşağıdakilerden hangisi çocuklarda kullanılan psikolojik değerlendirme yöntemleri arasında <u>yer almaz</u>?
    - A-) Gözlem
    - B-) Görüşme
    - C-) Testler
...
16. Okul ortamında arkadaşları ile sorun yaşayan bir çocuğun değerlendirmesinde hangi yöntem öncelikli olarak tercih edilmelidir?
...
17. Doğal ortamda gelişen, ön hazırlık yapılmadan gerçekleştirilen ve hangi konuların ele alınacağının görüşme sırasında şekillendiği psikolojik değerlendirme yöntemi aşağıdakilerden hangisidir?
    - A-) Yaratıcı drama
    - B-) Yarı yapılandırılmış görüşme
    - C-) Yapılandırılmış görüşme
    - **Cevap D-) Yapılandırılmamış görüşme**
    - E-) Psikodrama
...
18. Gözlemcinin sürece aktif biçimde katıldığı, olaylara ya da etkileşimlere doğrudan dahil olduğu gözlem yöntemi aşağıdakilerden hangisidir?
    - **Cevap A-) Katılımlı gözlem**
    - B-) Katılımsız gözlem
    - C-) Yapılandırılmış gözlem
    - D-) Yapılandırılmamış gözlem
    - E-) Dramatik gözlem

    > **Açıklama:** Katılımlı gözlem, gözlemcinin sürece aktif biçimde katıldığı ve olaylara veya etkileşimlere doğrudan dahil olduğu gözlem yöntemidir.

    <hr />

### 1.6.2.2. Görüşme

19. Aşağıdakilerden hangisi yüz yüze yapılan değerlendirme görüşmelerinin faydalarından <u>değildir</u>?
    ...
    - D-) Çocuğun sözel olmayan iletişiminin gözlenebilmesi
    - E-) Çocuğun davranışlarının gözlenebilmesi

    > **Açıklama:** Yüz yüze yapılan değerlendirme görüşmeleri sırasında çocuğun giyim kuşamı, sözel ve sözel olmayan iletişimi ve genel davranışları gözlenebilir. Ancak arkadaşları ile ilişkilerinin gözlenebilmesi bu görüşmelerin faydalarından değildir; bu, genellikle okul gözlemi gerektirir.

    <hr />

### 1.6.2.5. Portföy ve Dokümanlar

20. Değerlendirmede çocukla ilgili eski kayıtları ve değerlendirme raporlarını bulup onları kullanan bir ruh sağlığı uzmanı hangi değerlendirme yöntemini kullanmış olur?
    - A-) Görüşme
    - B-) Katılımlı gözlem
    - **Cevap C-) Dokümanları inceleme**
    - D-) Rol yapma yöntemi
    - E-) Katılımsız gözlem

    > **Açıklama:** Bireyin belirli bir alandaki gelişimini veya mevcut durumunu yansıtan ürün ve çalışmaların incelendiği değerlendirme yöntemi portföy veya dokümanları incelemedir. Eski kayıtları kullanmak bu yönteme girer.

    <hr />

### 1.6. Değerlendirme

21. Değerlendirme için aşağıdakilerden hangisi yanlıştır?
...
22. Aşağıdakilerden hangisi çocukları ölçme ve değerlendirmede erişkinlerden <u>farklı değildir</u>?
...
23. Aşağıdakilerden hangileri çocukların ölçme ve değerlendirmesinde erişkinlerden farklı olarak dikkate alınmalıdır?
I. Ölçüm aracının geçerli olması
II. Ebeveynlerinden bilgi alınması
III. Oyun ve hikaye gibi yöntemler kullanılması
IV. Etiketleyici bir dil kullanılmaması
    - A-) II ve IV
    - **Cevap B-) II ve III**
    - C-) I ve II
    - D-) I ve IV
...
### 1.2. Ölçmenin İşlevleri

24. Aşağıdakilerden hangisi psikolojik testlerin işlevlerinden biri <u>değildir</u>?
    - A-) Bireylerin seçilmesi
    - B-) Bireylerin sınıflandırılması
    - C-) Uygulanan yöntemlerin değerlendirilmesi
    - **Cevap D-) Psikoterapötik müdahalelerin doğrudan uygulanması**
    - E-) Bilimsel araştırmalarda değişkenlerin kontrolü

    > **Açıklama:** Psikolojik testlerin temel işlevleri; bireylerin seçimi, sınıflandırılması ve uygulanan yöntemlerin değerlendirilmesidir. Testler, değerlendirme sürecini kolaylaştırır ancak psikoterapötik müdahalelerin doğrudan uygulanması testlerin bir işlevi değildir.

    <hr />

### 1.6. Psikolojik Değerlendirme Yöntemleri

25. Ölçüm araçları aşağıdakilerden hangi şekilde kullanılmalıdır?
...
26. Çocuklarda ölçme işlemi için yukarıdakilerden hangileri doğrudur?
I. Çocuğun dikkat süresi kısa olduğu için ölçüme çocuk odaya girdikten sonra hemen başlamak gerekir.
II. Çocukla ilgili yeterli bilgi toplayıp bir kanaat oluştuktan sonra test istenmelidir.
III. Testin yapılma nedeni ve içeriği ile ilgili bilgi test yapılmadan önce verilmelidir.
IV. Çocuk hastayken test yapılması daha doğru bilgi verir.
    - A-) I ve II
    - B-) I ve IV
    - **Cevap C-) II ve III**
    - D-) II ve IV
...
27. Ölçme işlemi öncesinde yapılması gerekenler hangileridir?
I. Çocuğun sağlık durumunun öğrenilmesi
II. Çocukla uzun süre oyun oynanması
III. Çocuğun velisiyle uzun süre sohbet edilmesi
IV. Çocuğa yapılacak işlem hakkında bilgi verilmesi
    - A-) III ve IV
    - B-) I ve III
    - C-) I ve II
    - D-) II ve III
...
28. Çocuğun test performansını en uygun hale getirmek için hangisi yapılmaz?
...
29. Çocuğun test sırasında sıkıldığı, esnediği ve teste devam etmek istemediğini söylediği durumda ne yapılabilir?
...
30. Çocuğun dikkat süresinin kısa olması değerlendirmede hangi değişikliğe neden <u>olmaz</u>?
...
31. Aşağıdakilerden hangisi psikolojik formülasyonda kullanılan 4P formülasyonunun bileşenleri arasında <u>yer almaz</u>?
    - A-) Hazırlayıcı faktörler
    - B-) Başlatıcı faktörler
    - C-) Sürdürücü faktörler
    - D-) Koruyucu faktörler
    - **Cevap E-) Güvenilir faktörler**

    > **Açıklama:** Psikolojik formülasyonda kullanılan 4P modeli Hazırlayıcı (Predisposing), Başlatıcı (Precipitating), Sürdürücü (Perpetuating) ve Koruyucu (Protective) faktörleri içerir. Güvenilir faktörler bu bileşenler arasında yer almaz.
...
### 2.1.4. Kullanım Rahatlığı (Kullanışlılık)

2. Aşağıdakilerden hangisi ölçeğin kullanım rahatlığı ile ilgili <u>değildir</u>?
    - A-) Kolay uygulanması
    - B-) Kolay puanlanması
    - C-) Ucuz olması
    - **Cevap D-) Farklı ölçümlerde tutarlı sonuçlar vermesi**
    - E-) Kolay yorumlanması

    > **Açıklama:** Kullanım rahatlığı (Kullanışlılık); kolay uygulama, kolay puanlama, kolay yorumlama ve düşük maliyet gibi pratik özelliklerle ilgilidir. Farklı ölçümlerde tutarlı sonuçlar vermesi ise <b>güvenirlik</b> ile ilgili bir özelliktir.

    <hr />

### 2.1.2. Güvenirlik

3. Bir testin güvenirliğinin hesaplanmasında hangisi <u>kullanılmaz</u>?
    - A-) Testin farklı ölçümlerde benzer sonuçlar vermesi
    - B-) Testin farklı değerlendiricilerde benzer sonuçlar vermesi
    - C-) Paralel formların benzer sonuçlar vermesi
    - D-) Maddelerin tutarlı bir şekilde aynı değişkeni değerlendirmesi
    - **Cevap E-) Testin ölçüt ile benzer sonuçlar vermesi**
...
4. Aşağıdakilerden hangisi bir ölçüm aracının geçerliğinin hesaplanmasında kullanılır?
    - A-) İç tutarlılık
    - B-) Yüzdelik normlar
...
### 2.1.3.2. Yapı Geçerliği

5. Ölçeğin yapı geçerliği için hangi hesaplama yapılır?
    - **Cevap A-) Faktör analizi**
    - B-) Ölçütle karşılaştırılması
    - C-) İç tutarlık
...
6. Kapsam geçerliği hesaplanmasında aşağıdakilerden hangisi kullanılır?
    - A-) Yüzdelik normlar
    - B-) İç tutarlık
    - **Cevap C-) Belirtke tablosu**
    - D-) Faktör analizi
...
7. Ölçeğin kapsam geçerliği nedir?
    ...
    - E-) Ölçüm aracının farklı değerlendiricilerde tutarlı bir şekilde benzer sonuçlar vermesi

    > **Açıklama:** Kapsam geçerliği, ölçüm aracının, <b>ölçmesi amaçlanan değişkeni ve onunla ilişkili özellikleri ne ölçüde kapsadığını</b> ifade eder.

    <hr />

### 2.1.3.2. Yapı Geçerliği

8. Yapı geçerliğinin tanımı aşağıdakilerden hangisidir?
...
9. Norm dayanaklı test ve değerlendirmenin amacı aşağıdakilerden hangisidir?
...
10. Normlarla ilgili ifadelerden hangisi doğrudur?
...
11. Hangi testler için yaş normlarının mevcut olması önemlidir?
    - A-) Likert tipi ölçekler
    - **Cevap B-) Gelişim testleri**
    - C-) Kişilik testleri
...
### 2.1.1.2. Sınıf Normları

12. Sınıf normları hangi amaçla kullanılır?
    - **Cevap A-) Çocuğun sınıfın ortalama beceri düzeyine göre durumunun belirlenmesi**
    - B-) Farklı yaşlardaki çocukların beceri seviyesinin belirlenmesi
    - C-) Test edilen bireyin norm grubuna göre yüzdesinin belirlenmesi
    - D-) Testlerin bir sınıfın tamamına uygulanabilmesi
    - E-) Sınıflardaki çocukların başarısının artırılması

    > **Açıklama:** Sınıf normları, bir öğrencinin bulunduğu sınıf düzeyine göre gösterdiği performansı, <b>sınıfın ortalama beceri düzeyiyle karşılaştırarak değerlendirmeye</b> olanak sağlar.

    <hr />

### 2.1.2. Güvenirlik

13. Bir ölçüm aracının güvenirliği hakkında aşağıdaki ifadelerden hangisi doğrudur?
    ...
    - **Cevap D-) Bir ölçüm aracının aynı niteliği ölçtüğü farklı ölçümlerde tutarlı sonuç vermelidir.**
    - E-) Bir ölçüm aracının aynı niteliği ölçen paralel formları farklı sonuçlar vermelidir.
...
### 2.1.2. Güvenirlik Yöntemleri

14. Yukarıdakilerden hangileri bir ölçüm aracının güvenirlik analizi yöntemlerindendir?
I. İç tutarlılık
II. Yüzdelik normlar
III. Faktör analizi
IV. Paralel formlar
...
15. Aşağıdakilerden hangisi bir ölçüm aracının güvenirliğini arttıran faktörlerden biri <u>değildir</u>?
...
### 3.1.6. Bakımveren ve Çevreden Bilgi Almanın Önemi

1. Bir çocuğun gelişimsel değerlendirmesinden önce aşağıdakilerden hangisi yapılmalıdır?
...
2. Çocukların dili kullanma becerilerinin sınırlı olması ölçme ve değerlendirmede nasıl bir farklılığa neden olur?
...
3. Çocukların psikolojik değerlendirmesi ile ilgili aşağıdaki ifadelerden hangisi doğrudur?
    - A-) Çocukların değerlendirmesinde tek bilgi kaynağı çocuktur.
    - B-) Çocukların dili kullanma becerileri, erişkinlerle aynıdır.
    - **Cevap C-) Çocuklar, yaşadıkları sorunların bazılarını fark edecek deneyime sahip değildir.**
    - D-) Çocuklar, değerlendirmedeki etiketleyici dilden olumlu etkilenir.
    - E-) Değerlendirmeye gelmeden önce çocuğa değerlendirme hakkında bilgi verilmemelidir.

    > **Açıklama:** Çocuklar, yaşadıkları sorunların bazılarını fark edecek deneyime sahip değildir, çünkü çevre algıları ve farkındalık düzeyleri sınırlıdır. Bu nedenle, değerlendiricinin dolaylı gözlemlere ve üçüncü taraf raporlarına ihtiyacı vardır.

    <hr />

### 3.1. Ölçme ve Değerlendirmede Çocukluk Dönemine Özel Farklılıklar

4. Çocuğun bireysel farklılıkları değerlendirme sürecinde nasıl dikkate alınmalıdır?
...
5. Aşağıdakilerden hangisi çocukların değerlendirilmesinde göz önünde bulundurulması gereken faktörlerden biri <u>değildir</u>?
...
6. Aşağıdakilerden hangileri çocukların ölçme ve değerlendirmesinde erişkinlerden farklı olarak dikkate alınmalıdır?
I. Ölçüm aracının geçerli olması
II. Ebeveynlerinden bilgi alınması
III. Oyun ve hikaye gibi yöntemler kullanılması
IV. Etiketleyici bir dil kullanılmaması
...
7. Aşağıdakilerden hangisi çocukları ölçme ve değerlendirmede erişkinlerden farklı olarak dikkat edilmesi gereken özelliklerden biri <u>değildir</u>?
    - A-) Beceri gelişimi için kritik dönemlerin olması
...
8. Çocuğun dikkat süresinin kısa olması değerlendirmede hangi değişikliğe neden <u>olmaz</u>?
    - A-) Performansının yakından takip edilmesini gerektirir.
    - B-) Ölçme işlemini çocuğun keyif alacağı bir formatta sunmayı gerektirir.
    - C-) İşlem süresi uzarsa mola verilmesi gibi esneklikleri gerektirir.
    - **Cevap D-) Çocuğa saygılı davranılmasını gerektirir.**
...
9. Çocuğun test performansını en uygun hale getirmek için hangisi yapılmaz?
    - **Cevap A-) Test sonrası sonuç hakkında bilgi verilir**
    - B-) Isınma faaliyeti kullanılır
    - C-) Odanın düzeni çocuğa uygun hale getirilir
    - D-) Teste yumuşak geçiş yapılır
...
### 3.2.1. Çocuk Değerlendirmesinde Bütüncül ve Sistematik Yaklaşım

10. Çocuklarda ölçme işlemi için aşağıdakilerden hangisi doğrudur?
...
11. Çocuğun dikkat süresi kısa olduğu için değerlendirmede hangi uygulamaya dikkat edilmesi gerekir?
...
12. Çocuğun test sırasında sıkıldığı, esnediği ve teste devam etmek istemediğini söylediği durumda ne yapılabilir?
    - A-) Annesinden çocuğunu uyarması istenir.
    - **Cevap B-) Başka bir gün tamamlanmak üzere test ara verilir.**
    - C-) Çocuğa testi bitirirse ödül verileceği söylenir.
    - D-) Test bu şekilde tamamlanır.
...
### 3.1.12. Güçlü Yönlere Odaklanan Destekleyici Yaklaşım

13. Çocukların psikolojik değerlendirmesi ile ilgili aşağıdaki ifadelerden hangileri yanlıştır?
I. Değerlendirmede çocuğun eksiklerine odaklanılması gerekir.
...
### 3.1.7. Bilgilendirme ile İş Birliği Sağlama

14. Ölçüm öncesi yapılacak işlem hakkında neden bilgi verilmelidir?
    - **Cevap A-) Çocuğun uygun performans sergilemesi için**
    - B-) Çocukla arkadaş olmak için
    - C-) Ölçeğin iç tutarlılığını yükseltmek için
...
15. Çocuklarda ölçme işlemi öncesinde ısınma faaliyeti neden yapılır?
...
16. Çocuklarda ölçme işlemi sırasında çocuğun ölçüme uyumunun artırılabilmesi için yukarıdakilerden hangileri yapılabilir?
I. Ölçme işlemi sırasında başka şeylerle meşgul olmak
II. Ölçme işlemi sırasında çocuğun velisinin çocuğun doğru cevaplarını takdir etmesini istemek
III. Ölçüm öncesinde çocukla kısa bir ısınma faaliyeti yapmak
IV. Gerekli durumlarda ölçme işlemine ara vermek
    - A-) II ve III
    - B-) I ve II
    - **Cevap C-) III ve IV**
    - D-) I ve III
...
### 3.2.7. Çocuğun Seansa Uyumunun Yönetilmesi

17. Değerlendiricinin çocukla spontane olarak oyun oynayarak çocuğu gözlemlediği değerlendirme yöntemine verilen ad aşağıdakilerden hangisidir?
    - A-) Yapılandırılmış katılımlı gözlem
    - **Cevap B-) Yapılandırılmamış katılımlı gözlem**
    - C-) Yapılandırılmış katılımsız gözlem
    - D-) Portföy inceleme
...
### 3.1.13. Ölçmenin Kalitesi ve Klinik Karar Üzerindeki Etkisi

18. Çocuğun gelişimsel değerlendirmesinde yapılacak hatanın fark edilebilmesi için ne yapılmalıdır?
    - A-) Değerlendirme öncesi veliden bilgi alınmaması
    - **Cevap B-) Çocukla kısa bir süre oyun oynayarak çocuğun gelişimsel seviyesi hakkında fikir sahibi olunması**
    - C-) Her seferinde aynı ölçüm aracının kullanılması
...
19. Çocukla etkili bir ilişki kurmak, değerlendirme sürecinde neden önemlidir?
...
### 3.1.4. Gelişim Evrelerine Göre Önceliklerin Belirlenmesi

20. Psikolojik değerlendirmede, çocuklar minik yetişkinler olarak görülmemeli; gelişimsel özellikleri dikkate alınarak yaklaşılmalıdır. Bu ilke, aşağıdakilerden hangisinin önemini vurgular?
    - A-) Ölçüm aracının ucuz olmasının
    - B-) Uygulayıcının yetkinliğinin
    - **Cevap C-) Gelişim dönemlerine göre önceliklerin belirlenmesinin**
...
21. Aşağıdakilerden hangisi çocukla yapılan ölçmede dikkat edilmesi gereken etik ilkelerden biri <u>değildir</u>?
    - A-) Çocuğun mahremiyetine saygı göstermek
    - B-) Çocuğa saygı göstermek ve nazik ifadeler kullanmak
    - C-) Gönüllülük ilkesine uymak
...
22. Aşağıdakilerden hangisi, psikometrik ölçme araçlarının çocuklara uygulanmasında dikkat edilmesi gerekenlerden biri <u>değildir</u>?
...
23. Aşağıdakilerden hangisi çocuğun değerlendirme sürecinde motivasyonunu artırmak için kullanılabilir?
...
24. Çocuklarda ölçme işlemi öncesinde ısınma faaliyeti kullanılmasının sebebi nedir?
...
25. Aşağıdaki ifadelerden hangisi gelişimsel değerlendirme konusunda <u>yanlıştır</u>?
...
26. Aşağıdakilerden hangisi ruh sağlığı alanında ölçek kullanımının faydaları arasındadır?
...
### 3.2.5 Teste Yumuşak Geçiş Yapılması

27. Psikolojik değerlendirme sürecinde 'Teste Yumuşak Geçiş Yapılması'nın temel amacı nedir?
    - A-) Çocuğun test materyallerini önceden görmesini sağlamak.
    - B-) Değerlendiricinin dinlenmesine zaman tanımak.
    - **Cevap C-) Çocuğun performans kaygısı yaşamasını önlemek ve motivasyonunu artırmak.**
...
28. Bir çocuğun değerlendirme sürecinde 'hipotez oluşturma' neden önemlidir?
...
2. Ölçme ve değerlendirme raporu yazılmasının amaçlarından biri <u>değildir</u>?
...
3. Ölçme ve değerlendirme sonucunun raporlanması neden önemlidir?
...
4. Aşağıdakilerden hangisi psikolojik değerlendirme raporlarının içeriğini oluşturmaz?
...
### 4.3.1. Başlık ve Tanımlayıcı Bilgiler

5. Değerlendirme raporunun tanımlayıcı bilgiler kısmında hangi bilgi <u>bulunmaz</u>?
...
### 4.3.6. Özet ve Öneriler

6. Değerlendirme raporunun en önemli ve kendi içinde bir bütün teşkil etmesi gereken kısmı hangisidir?
...
7. Değerlendirme raporunun özet ve öneriler kısmı için hangisi doğrudur?
...
8. Psikolojik değerlendirme raporlarının içeriği hakkında aşağıdakilerden hangisi doğrudur?
...
### 4.3.5. Bulgular ve Yorumlar

9. Aşağıdakilerden hangisi değerlendirme raporunun 'Bulgular ve Yorumlar' kısmında yer alması gereken bir bilgidir?
    - A-) Çocuğun doğum tarihi
    - B-) Raporu talep eden kişinin unvanı
    - **Cevap C-) Uygulanan ölçüm araçlarının sonuçları ve çocuğun test davranışı**
...
### 4.3.3. Geçmiş Bilgisi

10. Psikolojik değerlendirme raporlarının 'Geçmiş Bilgisi' bölümünde hangi bilgi bulunur?
...
11. Ölçme ve değerlendirme yapan biri etik olarak hangi yeterliliği taşımalıdır?
    - A-) Ölçek geliştirme becerisi
    - B-) Esprili olma
    - C-) Sükûnet
...
12. Ölçme ve değerlendirme konusunda etik çalışan bir uygulayıcı hangisini yapar?
    - A-) Yeterli bilgisi olmadığı testleri uygulayarak para kazanır.
    - **Cevap B-) Standarda uygun olmayan uygulamayı sonuç raporunda yazar.**
    - C-) Test prosedürü konusunda çocuğa yalan söyler.
...
13. Ölçme ve değerlendirme raporları için hangisi yanlıştır?
    - A-) Rapor etiketleyici ifadeler içermemelidir.
    - B-) Raporun dili kime ve ne amaçla yazıldığına göre farklılık gösterir.
    - C-) Adli amaçla rapor yazılabilir.
...
14. Aşağıdakilerden hangisi bir psikolojik değerlendirme raporunda yer alan içerik öğeleriyle ilgili <u>yanlıştır</u>?
...
15. Aşağıdakilerden hangisi, ölçme ve değerlendirme sürecinde uyulması gereken etik kurallardan biri <u>değildir</u>?
...
### 4.7.6. Sonuçların Açıklanması

16. Değerlendirme raporunun içeriği konusunda aileye geribildirim verilirken öncelikli olarak neye dikkat edilmelidir?
...
17. Aşağıdakilerden hangisi ölçme ve değerlendirme yaptıran kişinin haklarından <u>değildir</u>?
...
18. Değerlendirme raporunun içeriği konusunda aileye bilgi verilirken özellikle neye dikkat edilmelidir?
...
### 4.7.5. Sonuçların Yorumlanması

19. Psikolojik test sonuçları yorumlanırken hangi ilke esastır?
    - A-) Yorumlar sadece toplam IQ puanına odaklanmalıdır.
    - **Cevap B-) Test davranışı test sonucunun güvenilir olup olmadığını belirler.**
    - C-) IQ puanı 100 olan çocuk üstün zekalı olarak yorumlanır.
...
### 4.2.2. Psikoeğitsel Değerlendirme Raporları

20. Psikoeğitsel raporlar, aşağıdakilerden hangisine özellikle odaklanır?
    - A-) Ceza ehliyeti ile ilgili değerlendirme yapılmasına
    - **Cevap B-) Eğitim destek hizmetlerinin planlanmasına**
    - C-) Bireyin yasal sorumluluklarının analiz edilmesine
...
21. Adli psikolojik değerlendirme raporlarının temel özelliklerinden biri <u>değildir</u>?
    - A-) Tarafsız ve nesnel bir yaklaşım sergilenmesi
    - B-) Sorulan hukuki sorulara odaklanması
    - C-) Çoklu kaynaklardan elde edilen verilerin kullanılması
...
22. Aşağıdakilerden hangisi, tüm rapor türlerinde ortak olarak bulunması gereken bir özelliktir?
...
23. Psikolojik değerlendirme raporlarının içeriği konusunda aşağıdaki ifadelerden hangisi yanlıştır?
...
24. Aşağıdakilerden hangisi, test seçimi sırasında etik olarak dikkat edilmesi gereken bir ilkedir?
    - **Cevap A-) Testin geçerlik ve güvenirlik düzeyinin yüksek olması**
    - B-) Testin maliyetinin düşük olması
    - C-) Testin çoklu dil seçeneğine sahip olması
...
25. Aşağıdakilerden hangisi, uygulayıcının etik olarak 'Yeterlik' ilkesi kapsamında dikkat etmesi gereken bir durumdur?
...
2. Normal gelişim ile ilgili aşağıdaki ifadelerden hangisi doğrudur?
    - A-) Çocuğun gelişimi, her zaman aynı persentil değerinde kalır.
    - **Cevap B-) Normal gelişim, çocuğun genetik özellikleri ve çevresinden etkilenir.**
    - C-) Normal gelişim, her çocukta aynı yörüngeyi takip eder.
...
### 5.3. Gelişimsel Problemler

3. Aşağıdakilerden hangisi gelişimsel bir sorunu <u>değildir</u>?
    - A-) Gelişim gecikmesi
...
7. Gelişimsel değerlendirme sonucunda gelişim düzeyi yaklaşık 2 yaş olarak belirlenen 5 yaşındaki çocuk için aşağıdakilerden hangisi doğrudur?
...
### 5.4. Gelişimsel Değerlendirmenin Amaçları

8. Aşağıdakilerden hangisi gelişimsel değerlendirmenin amaçlarından biri <u>değildir</u>?
    - A-) Çocuğun gelişim düzeyinin belirlenip ihtiyaçlarının saptanması
    - B-) Çocuğun gelişiminin izlenmesi
    - C-) Çocuğa yönelik müdahalelerin etkinliğinin değerlendirilmesi
    - D-) Çocuğun davranışlarının gelişimsel bağlamda incelenmesi
    - **Cevap E-) Çocuğa psikiyatrik bozukluk tanısı konması**

    > **Açıklama:** Gelişimsel değerlendirmenin amaçları arasında çocuğun gelişim düzeyini belirleme, takibini yapma ve müdahale etkinliğini değerlendirme yer alır. Ancak, gelişimsel değerlendirme testleri genellikle <b>hastalık tanısı koyma</b> (psikiyatrik bozukluk tanısı konması) amacıyla <u>kullanılmaz</u>.

    <hr />

### 5.4.3. Gelişimsel Değerlendirme Sonuçlarının Yorumlanması

9. Gelişimsel değerlendirme sonuçlarının yorumlanması konusunda aşağıdakilerden hangisi doğrudur?
...
10. Aşağıdakilerden hangisi Ankara Gelişim Tarama Envanteri’nin özellikleri arasında <u>yer almaz</u>?
...
11. Aşağıdakilerden hangisi ülkemizde kullanılan gelişim değerlendirme araçlarından <u>değildir</u>?
...
1. Aşağıdakilerden hangisi zekânın kapsadığı bilişsel beceriler arasında <u>yer almaz</u>?
    - A-) Bilgiyi kazanma ve öğrendiklerini uygulama
    - B-) Plan yapabilme
    - C-) Yargılama ve problem çözme
    - D-) Dikkat
    - **Cevap E-) Duygudurum**
...
2. Aşağıdakilerden hangileri zekânın kapsadığı becerilere dahil <u>değildir</u>?
I. Bilgiyi kazanma ve öğrendiklerini uygulama
II. Plan yapabilme
III. Duygu durum
IV. Denge
    - **Cevap A-) III ve IV**
...
### 6.5.3. Küme Etmen Kuramı (Thurstone)

4. Aşağıdakilerden hangisi Küme Etmeni Kuramına göre zekanın kapsamına giren becerilerdendir?
    - A-) Akıcı zeka
    - **Cevap B-) Sözel kavrama**
    - C-) Kişilerarası zeka
    - D-) Kristalize zeka
    - E-) Müziksel zeka
...
### 6.5.4. Çoklu Zekâ Kuramı (Gardner)

5. Aşağıdakilerden hangisi Gardner’in Çoklu Zekâ Kuramında yer alan zekâ alanlarından biridir?
    - A-) Kristalize zekâ
    - B-) Algısal Hız
    - C-) Sayısal yetenek
...
6. Aşağıdakilerden hangileri Gardner’in Çoklu Zekâ Kuramında yer alan zekâ alanlarındandır?
I. Kristalize zekâ
...
### 6.6. Zekâ ile İlgili Temel Özellikler

7. Aşağıdakilerden hangisi zekanın ölçülmesi konusunda doğrudur?
    - A-) Zeka puanları bebeklikten itibaren değişmez.
    - B-) Zekanın kalıtsallığı yaşla birlikte azalır.
    - C-) Zeka testleri tek başına tanı veya karar verme amacı taşımalıdır.
...
8. Aşağıdaki ifadelerden hangisi zekanın ölçülmesi konusunda doğru değildir?
    - A-) Yaşamın ilk iki yılında zeka ölçümü çocuğun gelişimsel değerlendirmesinden ibarettir.
    - B-) Küçük yaşlarda elde edilen IQ puanlarının devamlılığı düşüktür.
    - C-) Zeka bölümü, zeka yaşının kronolojik yaşa bölünüp 100 ile çarpılmasıyla elde edilir.
...
### 6.9. Ülkemizde Sık Kullanılan Zekâ Testleri

9. Aşağıdakilerden hangisi Wechsler Çocuklar için Zeka Ölçeği-R’ın alt testlerinden <u>değildir</u>?
    - A-) Sözcük bulma
    - B-) Küplerle desen
    - C-) Şifre
    - D-) Labirentler
...
10. İşitme problemi olan bir çocuğun değerlendirilmesinde hangi zeka testi kullanılmalıdır?
...
11. Zeka testinin yorumlanması için hangisi doğru değildir?
...
12. Aşağıdakilerden hangisi zeka testi <u>değildir</u>?
...
### 7.4. Otizm Spektrum Bozuklukları (OSB) Değerlendirmesi

3. Aşağıdaki ölçeklerden hangisi otizm spektrum bozukluklarının değerlendirmesinde kullanılan ölçeklerden biri <u>değildir</u>?
    - A-) Gilliam Otistik Bozukluk Derecelendirme Ölçeği
    - B-) Otizm Davranış Kontrol Listesi
    - C-) Çocukluk Çağı Otizm Derecelendirme Ölçeği
...
### 7.2.1. DEHB Klinik Değerlendirme Ölçekleri

4. Aşağıdakilerden hangisi dikkat eksikliği hiperaktivite bozukluğu (DEHB)’nun değerlendirilmesinde kullanılan ölçeklerden <u>değildir</u>?
...
5. Aşağıdakilerden hangisi DEHB’nin değerlendirilmesinde kullanılan ölçeklerdendir?
    ...
    > **Açıklama:** <b>Conners Anababa Değerlendirme Ölçekleri</b>, DEHB tanısı ve takibinde yaygın olarak kullanılan bir ölçek grubudur.

    <hr />

### 7.2.3. Dikkat Bileşenleri

6. Dağıtıcıları görmezden gelip belli bir uyarana odaklanabilmeye verilen ad aşağıdakilerden hangisidir?
    - A-) Sürdürülen dikkat
    - B-) Bölünmüş dikkat
    - C-) Görsel bellek
    - D-) Dikkati kaydırma
...
### 7.2.2. DEHB Nöropsikolojik Testleri

7. Aşağıdakilerden hangisi dikkat testidir?
    - A-) Tematik Algı Testi
    - B-) Gazi Beier Cümle Tamamlama Testi
    - C-) Ankara Artikülasyon Testi
    - D-) Metropolitan Okul Olgunluğu Testi
    - **Cevap E-) Stroop Testi**

    > **Açıklama:** <b>Stroop Testi</b>, Dikkat Eksikliği ve Hiperaktivite Bozukluğu (DEHB) değerlendirmesinde kullanılan nöropsikolojik bir dikkat testidir.

    <hr />

### 7.1. Projektif Testler

8. Aşağıdakilerden hangisi projektif test <u>değildir</u>?
    - A-) Frankfurter Testi
    - B-) Rorschach Mürekkep Lekesi Testi
    - C-) Luisa Düss Psikanalitik Hikayeler Testi
...
### 7.3. Dil ve Konuşma Değerlendirmesi

9. Aşağıdakilerden hangisi dil-konuşma testidir?
    - A-) Stanford Binet Testi
    - B-) Metropolitan Okul Olgunluğu Testi
    - **Cevap C-) Ankara Artikülasyon Testi**
    - D-) Çocuklar için Algı Testi
    - E-) Frankfurter Testi

    > **Açıklama:** <b>Ankara Artikülasyon Testi</b>, dil ve konuşma bozukluklarının değerlendirilmesinde kullanılan bir araçtır.

    <hr />

### 7.4. Otizm Spektrum Bozuklukları (OSB) Değerlendirmesi

10. Aşağıdaki ölçeklerden hangisi değerlendirici tarafından çocuğu gözleyerek ve ebeveyne sorularak doldurulur?
    ...
    - D-) Güçler ve Güçlükler Anketi
    - E-) Conners Anababa Değerlendirme Ölçeği
...
11. Aşağıdakilerden hangisi Okul Öncesi Dönem Psikiyatrik Değerlendirme (PAPA) aracı için doğrudur?
...
### 8.1 Okul Öncesi Dönemde Kullanılan Değerlendirme Araçları

1. Okul öncesi dönemde yapılan değerlendirmenin en önemli yararı aşağıdakilerden hangisidir?
    - A-) Çocuğun gelecekteki mesleğini belirlemek
    - **Cevap B-) Gelişimsel gecikmeleri erken dönemde saptamak**
    - C-) Çocuğun akademik başarılarını puanlamak
...
### 8.1.3.1. Metropolitan Okul Olgunluğu Testi

2. Aşağıdakilerden hangisi Metropolitan Okul Olgunluğu Testi hakkında doğru bilgidir?
    - A-) Çocukların kaba motor becerilerini değerlendirir.
    - **Cevap B-) Çocukların ilkokula hazırlık düzeyini saptamak için kullanılır.**
    - C-) Eğitim ve süpervizyon olmadan uygulanabilir.
...
3. Metropolitan Okul Olgunluğu Testinin alt testlerinden <u>değildir</u>?
    - A-) Kelime anlama
    - B-) Kopya etme
    - C-) Genel bilgi
...
### 8.3.4.3. Özgül Öğrenme Bozukluğu (ÖÖB) Bataryası

4. Aşağıdakilerden hangisi Özgül Öğrenme Güçlüğü Test Bataryasının değerlendirdiği becerilerden <u>değildir</u>?
    - A-) Okuma
    - B-) Yazma
    - C-) Sağ-sol ayırt etme
    - D-) Lateralleşme
    - **Cevap E-) Sözel zeka**
...
5. Aşağıdakilerden hangisi özgül öğrenme güçlüğü test bataryasında yer alan testlerden <u>değildir</u>?
