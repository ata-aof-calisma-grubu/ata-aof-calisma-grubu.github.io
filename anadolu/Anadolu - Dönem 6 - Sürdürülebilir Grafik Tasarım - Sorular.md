# Sürdürülebilir Grafik Tasarım (Dönem 6) - Tüm Sorular

## Unite 1
1. 1\. İnsanlar tarafından yaratılan su kirliliği<br />2\. İnsanların kuraklık nedeniyle açlık ve susuzluk çekmesi<br />3\. İnsanlar tarafından yapılan avlanma faaliyetleri<br />Yukarıdaki ifadelerden hangisi/hangileri bir etkileşim (Koaksiyon) örneğidir?
    - A-) Yalnız I
    - B-) Yalnız II
    - **Cevap C-) Yalnız III**
    - D-) I ve II
    - E-) II ve III

    > **Açıklama:** Canlıların bir birleriyle ve çevreleriyle olan tüm ilişkilerinin yönü farklılıklar gösterdiği için Eylem (Aksiyon), Tepki (Reaksiyon) ve **Etkileşim (Koaksiyon)** olmak üzere ayrı ayrı tanımlanır. Bu ilişkileri, insanlar üzerinden örnekler vererek şöyle
    > tanımlayabiliriz:
    > • EYLEM; Çevrenin → Organizmalar üzerine olan etkileri. Örnek; İnsanların kuraklık nedeniyle açlık ve susuzluk çekmesi.
    > • REAKSİYON: Organizmaların → Çevre üzerine etkileri. Örnek; İnsanlar tarafından yaratılan çevre kirliliği.
    > **ETKİLEŞİM: Organizmaların → Organizmalarla etkileşimleri. Örnek; İnsanlar tarafından yapılan avlanma faaliyetleri.**

    <hr />
1. Yapay ekosistemlere en iyi örnek aşağıdakilerden hangisidir?
    - A-) Ormanlar
    - B-) Tarım alanları
    - C-) Çayırlar
    - **Cevap D-) Kentler**
    - E-) Gıda fabrikaları

    > **Açıklama:** **Yapay ekosistemler için ise verilebilecek en iyi örnek ise şehirlerdir.** Bunlar üzerindeki doğal ekosistem unsurlarının tamamen ortadan kaldırıldığı, insan eliyle yapay malzemeler kullanılarak doğal yüzeylerin kaplandığı, oluşturulan yapay yeşil alanların dahi insan desteğine ihtiyaç duyduğu, biyolojik çeşitliliğin en düşük seviyede olduğu, atıkların doğal süreçlerle dönüştürülmediği, hemen hemen su/gıda maddesi ve enerji gibi tüm yaşamsal gereksinimlerin dışarıdan karşılanmadığı takdirde varlığını sürdürmeyen yapay ekosistemlerdir.

    <hr />
1. Zaman içinde değişen ekolojik faktörlere bağlı olarak bir bölgedeki var olan türlerin ve habitatların sıralı olarak değişim sürecine ne ad verilir?
    - **Cevap A-) Ardıllık**
    - B-) Rekreasyon
    - C-) Reaksiyon
    - D-) Süreklilik
    - E-) Metamorfoz

    > **Açıklama:** **Ardıllık:** Zaman içinde değişen ekolojik faktörlere bağlı olarak bir bölgedeki var olan türlerin ve habitatların sıralı olarak değişim sürecidir.

    <hr />
1. Yarı doğal ekosistemler ile yapay ekosistemlerin en önemli ortak özelliği aşağıdakilerden hangisidir?
    - A-) Az sayıda canlı türü ve yetersiz ekolojik faktörü içerir**.**
    - B-) Genetik çeşitlilik çok düşüktür.
    - **Cevap C-) Tüm enerji ve biyolojik döngüler insan desteği olmaksızın gerçekleşemez.**
    - D-) Besin zincirleri basittir ve eksiktir.
    - E-) Ekolojik ardıllık ancak insan etkisi kalktıktan sonra ve yavaş gerçekleşir.

    > **Açıklama:** Yarı doğal ekosistemler ile yapay ekosistemlerin **en önemli ortak özelliği** sürdürülebilir olmaları için **insan müdahalesine** olan gereksinimleridir.

    <hr />
1. 1\. Doğal riskleri azaltma<br />2\. Bitkisel üretim<br />3\. Besin kaynağı ve gıda<br />4\. Zararlılara karşı direnç<br />5\. Tozlaşma ve tohum dağılımı<br />Yukarıdakilerden hangileri ekosistem hizmetlerinin 'düzenleme hizmetleri' başlığı altında yer alır?
    - A-) I ve III
    - B-) I ve V
    - C-) IV ve V
    - **Cevap D-) I, IV ve V**
    - E-) II, III ve IV

    > **Açıklama:** **Şekil 1.3'te görüldüğü gibi**
    > ![](https://ets.anadolu.edu.tr/storage/nfs/questions/1fd8a089e8e665c1cd4640c4191f71c9.png)

    <hr />
1. Sürdürülebilirliğin temel bileşenlerini destekleyen strateji aşağıdakilerden hangisidir?
    - A-) Ekonomik sürdürülebilirlik
    - **Cevap B-) Kültürel sürdürülebilirlik**
    - C-) Sosyo/ekonomik sürdürülebilirlik
    - D-) Ekolojik/Çevresel sürdürülebilirlik
    - E-) Sosyal sürdürülebilirlik

    > **Açıklama:** Sürdürülebilirlik temel olarak ekonomi ve ekoloji (çevre) bilimlerinin bakış açılarının, hem teorik hem de pratik olarak birleştirilmesine dayanan bir bakış açısına sahiptir. Ancak sürdürülebilir kalkınmaya yönelik birçok kaynakta bu yaklaşımın, ekolojik/çevresel ve ekonomik faydalarının yanı sıra kurumsal ve sosyal kazanımları da sağlanabilen ve birbirini tamamlayan **kültürel kapsamlı** bir gelişme süreci olduğu kabul edilmektedir. **Kültür bu üç temel sütunu desteklemek için etkili bir strateji olarak değerlendirilmektedir.**

    <hr />
1. İnsan yaşamının destekleyicisi olan ekosistemleri, yeniden üretme ve onların taşıma kapasitesini aşmayacak şekilde kullanarak insan kültürü ve yaşayan dünya arasındaki mevcut yıkıcı ilişkiyi dengeleme sorumluluğu olarak tanımlanan yeni yaklaşım aşağıdakilerden hangisidir?
    - **Cevap A-) Ekolojik ve çevresel sürdürülebilirlik**
    - B-) Ekolojik ayak izi
    - C-) Taşıma kapasitesi
    - D-) Sosyal sürdürebilirlik
    - E-) Ekonomik sürdürülebilirlik

    > **Açıklama:** İnsan faaliyetlerine açık olan ormanlar, sahiller, mera/çayırlar, tarım arazileri ve kentsel alanlar vb.ekosistem parçaları yoğun ve ekolojik taşıma kapasitesinin üzerinde kullanılarak tüketilmekte ya da değişime uğratılmaktadır. Bunun sonucunda bir yandan sosyoekonomik refah artarken diğer yandan iklim, su, gıda ve enerji kaynaklarının korunması ve kirlilik kontrolüne yönelik klasik yaklaşımlar, sürdürülemez hâle gelen sorunlara sistematik ve uzun vadeli çözümler yaratabilme konusunda yetersizkalmaktadır.
    > Böylece insan refahı ve ekosistem hizmetleri arasındaki bağların doğru tanımlanmasına ve yönetilmesine yönelik **yeni ekolojik ve çevresel yaklaşımlar benimsenmeye başlanmıştır.**
    > İşte bu kapsamda; insan yaşamının destekleyicisi olan ekosistemleri, yeniden üretme ve onların taşıma kapasitesini aşmayacak şekilde kullanarak insan kültürü ve yaşayan dünya arasındaki mevcut yıkıcı ilişkiyi dengeleme sorumluluğu olarak tanımlanan **ekolojik ve çevresel sürdürülebilirlik** yaklaşımı, sürdürülebilirlik gündeminde yerini almıştır.

    <hr />
1. Kaynakları verimli kullanarak ekolojik ve çevresel sorunları önemli ölçüde azaltırken insan refahını ve sosyal eşitliği iyileştiren ekonomi modeline ne ad verilir?
    - A-) Doğrusal Ekonomi
    - B-) Döngüsel Ekonomi
    - C-) Geri Dönüşüm Ekonomisi
    - D-) Sosyoekonomik Dönüşüm
    - **Cevap E-) Yeşil Ekonomi**

    > **Açıklama:** **Yeşil Ekonomi:** Kaynakları verimli kullanarak ekolojik ve çevresel sorunları önemli ölçüde azaltırken insan refahını ve sosyal eşitliği iyileştiren bir ekonomidir.

    <hr />
1. 1\. Sürdürülebilirlik sistemi düşüncesi<br />2\. Ağ oluşturma<br />3\. Bilgi yönetimi<br />4\. Yaratıcı uygulamalar<br />5\. Yansıtıcı uygulamalar<br />Yukarıdakilerden hangileri sürdürülebilirliğin toplumda yaygınlaşması için sosyal öğrenmenin organizasyonunda temel basamaklardandır?
    - A-) I, II ve III
    - B-) I, III ve V
    - C-) I, III ve IV
    - D-) II, II, IV ve V
    - **Cevap E-) I, II, III ve V**

    > **Açıklama:** rg

    <hr />
1. Sosyal öğrenme süreçlerinde, diğer insanların davranışlarının sonuçlarını gözlemleme amacını aşağıdakilerden hangisi kapsar?
    - A-) Dikkat çekme
    - B-) Davranışı fark etme
    - C-) Hatırlama
    - **Cevap D-) Motivasyon**
    - E-) Teklarlama

    > **Açıklama:** **Tablo 1.4'de görüldüğü gibi;**
    > **![](https://ets.anadolu.edu.tr/storage/nfs/questions/d72d738b849d87c0b43883ebae405199.png)**

    <hr />
1. Doğal kaynakların sorumlu kullanılarak ekosistemlerin sağlığı ve insan refahının uzun vadeli olarak birlikte korunması gerektiği anlayışını ifade eden kavram aşağıdakilerden hangisidir?
    - **Cevap A-) Sürdürülebilirlik**
    - B-) Ekoloji
    - C-) Çevre
    - D-) Ekosistem
    - E-) İnovasyon

    > **Açıklama:** **Sürdürülebilirlik Kavramı:** Doğal kaynakların sorumlu kullanılarak ekosistemlerin sağlığı ve insan refahının uzun vadeli olarak birlikte korunması gerektiği anlayışını ifade etmektedir.

    <hr />
1. Aşağıdaki seçeneklerden hangisi/hangileri ekoloji kavramı içinde değerlendirilir?<br />**I.**Organizmalar ve çevreleri arasındaki tüm ilişkileri kapsar. **II.** Çevrede fiziksel, kimyasal ve biyolojik bileşenler arasındaki etkileşimleri ifade eder. **III.** Canlıların yaşam döngüleri ve süreçlerini, organizmaların dağılımını, adaptasyonunu ve biyolojik çeşitliliği anlamaya odaklanır. **IV.**Çevreyi etkileyen iç ve dış faktörlere odaklanır. **V.** Canlı tür ve topluluklarının büyüklüğü, çeşitliliği ve aradaki rekabeti içerir.
    - A-) I, II ve IV
    - **Cevap B-) I, III ve V**
    - C-) II, III ve V
    - D-) I, IV ve V
    - E-) II, III ve IV

    > **Açıklama:** **EKOLOJİ:** Organizmalar ve çevreleri arasındaki tüm ilişkileri kapsar. Canlıların yaşam döngüleri ve süreçlerini, organizmaların dağılımını, adaptasyonunu ve biyolojik çeşitliliği anlamaya odaklanır. Canlı tür ve topluluklarının büyüklüğü, çeşitliliği ve aradaki rekabeti içerir. Ekosistemlerde gerçekleşen tüm doğal süreçleri ifade eder.

    <hr />
1. Aşağıdakilerden hangisi BİYOTİK FAKTÖRLER arasında yer alır?
    - A-) Su
    - B-) Toprak ve mineraller
    - **Cevap C-) Ayrıştırıcılar**
    - D-) Işık
    - E-) İklim

    > **Açıklama:** BİYOTİK FAKTÖRLER: Üreticiler (Bitkisel canlılar), Tüketiciler (Hayvansal canlılar), Ayrıştırıcılar (Bakteri ve mantarlar).

    <hr />
1. Aşağıdakilerden hangisi/hangileri **Yapay ve Yarı Doğal Ekosistemler** içinde yer alır?<br />**I.** Çok sayıda canlı türü ve yeterli ekolojik faktörleri içerir. **II.** Genetik çeşitlilik çok düşüktür. **III.** Canlılar arası besin zincirleri uzun ve karmaşıktır. **IV.** Tüm enerji ve biyolojik döngüler insan desteği olmaksızın gerçekleşemez. **V.** Ekolojik ardıllık hızla ve sürekli gerçekleşir.
    - A-) I, II ve IV
    - B-) II, III ve V
    - C-) III ve IV
    - **Cevap D-) II ve IV**
    - E-) III ve V

    > **Açıklama:** **Yapay ve Yarı Doğal Ekosistemler:** Az sayıda canlı türü ve yetersiz ekolojik faktörü içerir.Genetik çeşitlilik çok düşüktür. Tüm enerji ve biyolojik döngüler insan desteği olmaksızın gerçekleşemez. Pek çok canlı türü ortadan kaldırıldığı için besin zincirleri basittir ve eksiktir.Ekolojik ardıllık ancak insan etkisi kalktıktan sonra ve yavaş gerçekleşir. Yapay oldukları için sürdürülemezler.

    <hr />
1. Ekosistem hizmetlerinin temel mal ve hizmet akışlarına dönüştürülmesinde, aşağıdaki seçeneklerden hangisi **Düzenleme Hizmetleri** içinde yer alır?
    - A-) Yaşam alanı ve sığınak; Oksijen üretimi ve su döngüsü; Bitkisel üretim
    - B-) Enerji ve hammadde; Genetik kaynaklar; Tatlı su
    - C-) Yer ve çevre duygusu; Rekreasyon, estetik ve değerler; Doğa bilgisi, eğitim ve ilham
    - D-) Otlatma; Madde döngüsü; Manevi değerler
    - **Cevap E-) Tozlaşma ve tohum dağılımı; Doğal riskleri azaltma; Hastalıklar ve su arıtma**

    > **Açıklama:** **Düzenleme Hizmetleri:**• Doğal riskleri azaltma • İklim düzenleme • Zararlılara karşı direnç • Tozlaşma ve tohum dağılımı • Hastalıklar ve su arıtma

    <hr />
1. "**...**  temel olarak ekonomi ve ekoloji (çevre) bilimlerinin bakış açılarının, hem teorik hem de pratik olarak birleştirilmesine dayanan bir bakış açısına sahiptir" tanımlamasında yer alan boşluğa en uygun seçenek aşağıdakilerden hangisidir?
    - **Cevap A-) Sürdürülebilirlik**
    - B-) Ekosistem
    - C-) Değişim
    - D-) Kalkınma
    - E-) Çevre

    > **Açıklama:** **Sürdürülebilirlik** temel olarak ekonomi ve ekoloji (çevre) bilimlerinin bakış açılarının, hem teorik hem de pratik olarak birleştirilmesine dayanan bir bakış açısına sahiptir.

    <hr />
1. Her bireyin tükettiği malları üretmek ve üretilen atıkları özümsemek için gereken biyolojik olarak üretken ekosistemler (taşıma kapasitesi) üzerindeki insan etkisinin ölçüsü aşağıdakilerden hangisidir?
    - A-) Biyolojik Taşıma Kapasitesi
    - **Cevap B-) Ekolojik Ayak İzi**
    - C-) Ekolojik ve Çevresel Sürdürülebilirlik
    - D-) Ekolojik taşıma
    - E-) Çevresel yaklaşım

    > **Açıklama:** **Ekolojik Ayak İzi:** Her bireyin tükettiği malları üretmek ve üretilen atıkları özümsemek için gereken biyolojik olarak üretken ekosistemler (taşıma kapasitesi) üzerindeki insan etkisinin ölçüsüdür.

    <hr />
1. Kaynakları verimli kullanarak ekolojik ve çevresel sorunları önemli ölçüde azaltırken insan refahını ve sosyal eşitliği iyileştiren bir ekonomi aşağıdakilerden hangisidir?
    - A-) Doğrusal Ekonomi
    - B-) Döngüsel Ekonomi
    - **Cevap C-) Yeşil Ekonomi**
    - D-) Karma ekonomi
    - E-) Piyasa ekonomisi

    > **Açıklama:** **Yeşil Ekonomi:** Kaynakları verimli kullanarak ekolojik ve çevresel sorunları önemli ölçüde azaltırken insan refahını ve sosyal eşitliği iyileştiren bir ekonomidir.

    <hr />
1. "**... ... ,** farklı bakış açılarını daha iyi anlamak ve zaman içinde ortak eylemler ve yansıma süreçlerini geliştirmek için bireyler ya da gruplar arasındaki diyaloglardan öğrenmeyi işaret eder" tanımında yer alan boşluğa gelmesi en uygun seçenek aşağıdakilerden hangisidir?
    - A-) Sürdürülebilirlik bilinci
    - B-) Sosyal sürdürülebilirlik
    - C-) Sosyal dayanışma
    - **Cevap D-) Sosyal öğrenme**
    - E-) Ekosistem hizmetleri

    > **Açıklama:** **Sosyal** **öğrenme**, farklı bakış açılarını daha iyi anlamak ve zaman içinde ortak eylemler ve yansıma süreçlerini geliştirmek için bireyler ya da gruplar arasındaki diyaloglardan öğrenmeyi işaret eder.

    <hr />
1. Sürdürülebilirliğin toplumda yaygınlaşması için sosyal öğrenmenin organizasyonunda yer alan temel basamaklar aşağıdakilerden hangisi / hangileridir?<br />I. Sürdürülebilirlik sistemi düşüncesi II. Ağ oluşturma III. Diyalog IV. Bilgi yönetimi V.Yansıtıcı uygulamalar
    - A-) I
    - B-) I ve II
    - C-) II, III ve IV
    - D-) II, III, IV ve V
    - **Cevap E-) I, II, III, IV ve V**

    > **Açıklama:** Sürdürülebilirliğin toplumda yaygınlaşması için sosyal öğrenmenin organizasyonunda beş te- mel basamak vardır. Bunlar:
    > • Sürdürülebilirlik sistemi düşüncesi • Ağ oluşturma • Diyalog • Bilgi yönetimi• Yansıtıcı uygulamalar

    <hr />
## Unite 2
1. Aşağıdakilerden hangisi sürdürülebilirlik kavramı doğrultusunda, atık ya da kullanılmayan nesnenin sanat objesine dönüşme sürecinde, eğitim kurumlarında ders kapsamında öğretilen ve uygulamalarının yapıldığı, temel yöntemlerden biri **değildir?**
    - A-) Kolaj
    - B-) Asamblaj
    - C-) Akümülasyon
    - **Cevap D-) Dadaizm**
    - E-) Montaj

    > **Açıklama:** Sürdürülebilirlik kavramı doğrultusunda, atık ya da kullanılmayan nesnenin sanat objesine dönüşme sürecinde, eğitim kurumlarında ders kapsamında öğretilen ve uygulamalarının yapıldığı, temel yöntemler aşağıdaki gibi sıralanabilir.
    > Kolaj
    > Dekolaj
    > Asamblaj
    > Akümülasyon
    > Montaj

    <hr />
1. Kelime olarak bir araya getirme birleştirme anlamına gelen ve bir resim tekniği olan, elde var olan her türlü basılı, yazılı, çizili, resimsel malzemelerin bir yüzey üzerinde yeniden kurgulanmasıyla oluşturulan yöntem aşağıdakilerden hangisidir?
    - **Cevap A-) Kolaj**
    - B-) Pentür
    - C-) Dadaizm
    - D-) Akrilik
    - E-) Litografi

    > **Açıklama:** **Kolaj:** Kelime olarak bir araya getirme birleştirme anlamına gelir. Bir resim tekniğidir. Elde var olan her türlü basılı, yazılı, çizili, resimsel malzemelerin bir yüzey üzerinde yeniden kurgulanmasıyla oluşturulur.

    <hr />
1. Üst üste katmanlar hâlinde yerleştirilmiş afiş, resim ya da basılı kağıtların bazı bölümlerinin kesilip çıkarılmasıyla oluşturulan Fransızca kökenli *dekolaj* kelimesinin Türkçe anlamı aşağıdakilerden hangisidir?
    - A-) yapıştırmak
    - B-) çizilmek
    - C-) karalamak
    - **Cevap D-) sökülmek**
    - E-) iyileşmek

    > **Açıklama:** **Dekolaj:** “Sökülmek” anlamına gelen Fransızca bir kelimedir. Üst üste katmanlar hâlinde yerleştirilmiş afiş, resim ya da basılı kağıtların bazı bölümlerinin kesilip çıkarılmasıyla oluşturulur. Yapılışı kolajın tam aksine bir süreç içinde gerçekleştirildiği hâlde hem sanatsal anlatım hem de sonuç olarak kolajdan pek farklı değildir.

    <hr />
1. 3 boyutlu sanat objesinin, heykelin, görsel sanatlardaki geleneksel malzemeler yerine artıklar, kullanılmayan objeler ve hurdaların bir araya getirilmesiyle oluşturulması aşağıdakilerden hangisidir?
    - A-) Kolaj
    - B-) Asamblaj
    - C-) Dadaizm
    - **Cevap D-) Junk Sanat**
    - E-) Minimalizm

    > **Açıklama:** **Junk sanat:** (Çöp Sanatı), 3 boyutlu sanat objesinin, heykelin, görsel sanatlardaki geleneksel malzemeler yerine artıklar, kullanılmayan objeler ve hurdaların bir araya getirilmesiyle oluşturulması.

    <hr />
1. 3 boyutlu sanat objesinin, heykelin, görsel sanatlardaki geleneksel malzemeler yerine artıklar, kullanılmayan objeler ve hurdaların bir araya getirilmesiyle oluşturulmasına ne ad verilir?
    - A-) Asamblaj
    - B-) Pop-art
    - **Cevap C-) Junk sanat**
    - D-) Minimalizm
    - E-) Serigrafi

    > **Açıklama:** **Junk sanat:** (Çöp Sanatı), 3 boyutlu sanat objesinin, heykelin, görsel sanatlardaki geleneksel malzemeler yerine artıklar, kullanılmayan objeler ve hurdaların bir araya getirilmesiyle oluşturulması.

    <hr />
1. “Asamblaj” terimi ilk kez 1953’te hangi sanatçı tarafından güzel sanatlar bağlamında kelebek kanatlarıyla oluşturduğu kendi eserini tanımlamak için kullanmıştır?
    - **Cevap A-) Jean Dubuffet**
    - B-) Pablo Picasso
    - C-) Wolf Vostell
    - D-) Robert Rauschenberg
    - E-) George Braque

    > **Açıklama:** **Asamblaj:** Asamblaj tekniği kolajın üç boyutlu hâlidir. Sanatçılar 20. yüzyılın başlarında bu tekniği keşfetmeye başlamışlardır. “Asamblaj” terimi ilk kez 1953’te Jean Dubuffet tarafından güzel sanatlar bağlamında kelebek kanatlarıyla oluşturduğu kendi eserini tanımlamak için kullanmıştır. (Walker, J.A.,1992).

    <hr />
1. ![](https://ets.anadolu.edu.tr/storage/nfs/questions/fea7225ee952b02aef9ec08c0dbf0f39.png)<br />Almanya'da Kassel Fridericianum müzesinin bahçesinde, çevresi 7000 bazalt sütün yığınıyla doldurulmuş bir meşe ağacı fidanı dikerek, insanların doğaya ağaç ve bitki ekosistemine daha bağlı olmasının gerekliliğini, özellikle kentsel bölgelerde doğa ile yakınlığın sürekliliğinin korunmasının gerektiğini vurgulamaya çalışan sanatçı aşağıdakilerden hangisidir?
    - **Cevap A-) Joseph Beuys**
    - B-) Pablo Picasso
    - C-) Edward Munch
    - D-) Robert Rauschenberg
    - E-) Hans Haacke

    > **Açıklama:** Alman sanatçı Joshep Beuys’un ise ekolojik sanatla ilgili olarak yaptığı önemli projelerden birisi “7000 Oak” (7000 Meşe) projesidir. (Görsel 2.9) Kassel’de gerçekleştirdiği proje zamanla büyümüş ve geliştirilmiş bir projedir. “Beuys’s Acorns” (Beuys’un Meşe Palamutları) isimli proje için öncü olmuştur. Görsel 2.10’da sanatçı Kassel Fridericianum müzesinin bahçesinde, çevresi 7000 bazalt sütün yığınıyla doldurulmuş bir meşe ağacı fidanı dikerken görülmektedir. Projede insanların doğaya ağaç ve bitki ekosistemine daha bağlı olmasının gerekliliğini, özellikle kentsel bölgelerde doğa ile yakınlığın sürekliliğinin korunmasının gerektiğini vurgulamaktadır.

    <hr />
1. Yaratıcı yeniden kullanım olarak da bilinen, yan ürünleri, atık malzemeleri, işe yaramaz veya istenmeyen ürünleri, sanatsal değer veya çevresel değer gibi önceki durumundan daha nitelikli olarak algılanan yeni malzemelere veya ürünlere dönüştürme süreci aşağıdakilerden hangisidir?
    - A-) pop-art
    - B-) eko-dizayn
    - **Cevap C-) upcycling (ileri dönüşüm)**
    - D-) recycling (geri dönüşüm)
    - E-) biyo-sanat

    > **Açıklama:** **Upcycling:** “İleri dönüşüm”, yaratıcı yeniden kullanım olarak da bilinen ileri dönüşüm, yan ürünleri, atık malzemeleri, işe yaramaz veya istenmeyen ürünleri, sanatsal değer veya çevresel değer gibi önceki durumundan daha nitelikli olarak algılanan yeni malzemelere veya ürünlere dönüştürme sürecidir.

    <hr />
1. Simon Rodia’nın hurda metal, cam, seramik ve çeşitli bulduğu objelerle 1921-1954 yılları arasında inşa ettiği 17 yapıdan oluşan bir mimari yapı olan Los Angeles'taki Watts Towers, aşağıdakilerden hangisine dair önemli örneklerden biridir?
    - **Cevap A-) upcycling**
    - B-) eko-art
    - C-) dadaizm
    - D-) asamblaj
    - E-) kolaj

    > **Açıklama:** **Upcycling:** “İleri dönüşüm”, yaratıcı yeniden kullanım olarak da bilinen ileri dönüşüm, yan ürünleri, atık malzemeleri, işe yaramaz veya istenmeyen ürünleri, sanatsal değer veya çevresel değer gibi önceki durumundan daha nitelikli olarak algılanan yeni malzemelere veya ürünlere dönüştürme sürecidir. Los Angles’taki “Watts Towers” kuleleri ileri dönüşümün en eski örneklerinden birisidir. Simon Rodia’nın hurda metal, cam, seramik ve çeşitli bulduğu objelerle 1921-1954 yılları arasında inşa ettiği 17 yapıdan oluşan bir mimari yapıdır. Günümüzde ileri dönüşüm sanatını kullanan çok sayıda sanatçı vardır.

    <hr />
1. Sanatçıların biyoloji, canlı dokular, bakteriler, canlı organizmalar ve yaşam süreçleri ile çalıştığı sanat pratiği aşağıdakilerden hangisidir?
    - A-) ekolojik sanat
    - B-) eko dizayn
    - **Cevap C-) biyo-sanat**
    - D-) biyoloji
    - E-) arazi sanatı

    > **Açıklama:** **Biyo-Sanat:** Biyo-Sanat, 20. yüzyılın sonu ve 21. yüzyılın başında ortaya çıkmıştır. ***Biyo-Sanat*** çalışan sanatçılar canlı madde ile çalışsalar da maddenin canlı veya canlı olarak kabul edilebileceği aşamalar konusunda etik, sosyal ve estetik tartışmalar vardır. Sanat eserleri laboratuvarlarda, galerilerde veya sanatçı stüdyolarında üretilirken, bazı sanatçılar canlı dokular, bakteriler, canlı organizmalar ve yaşam süreçlerinin sadece görsellerini kullanarak eserlerini üretirler.
    > **Biyo-Sanat,** sanatçıların biyoloji, canlı dokular, bakteriler, canlı or-ganizmalar ve yaşam süreçleri ile çalıştığı bir sanat pratiğidir.

    <hr />
1. Sanayileşmenin getirdiği kirlilik, küresel ısınma, artan nüfus ve buna bağlı olarak artan tüketim, azalan kaynaklarımız ve üretimin sınırlılığı, aşağıdaki kavramlardan hangisinin her alanda daha yoğun biçimde kullanımını ortaya çıkarmıştır?
    - A-) İnovasyon
    - B-) Geri dönüşüm
    - **Cevap C-) Sürdürülebilirlik**
    - D-) Ekolojik Tasarım
    - E-) Globalleşme

    > **Açıklama:** Sanayileşmenin getirdiği kirlilik ve küresel ısınma da daha uzun vadede doğayı yaşanılmaz hâle getirmektedir. Bu bağlamda artan nüfus ve buna bağlı olarak artan tüketim, azalan kaynaklarımız ve üretimin sınırlılığı “sürdürülebilirlik” kavramının her alanda daha yoğun biçimde kullanımını ortaya çıkarmıştır.

    <hr />
1. Aşağıdakilerden hangisi küreselleşmenin en belirgin faydaları arasında **değildir?**
    - A-) Sürdürülebilir ekonomik kalkınma
    - B-) Yükselen yaşam standartları
    - C-) Teknolojik ilerleme
    - D-) Bilginin daha hızlı yayılması
    - **Cevap E-) Toplumsal eşit hakların yaygınlaşması**

    > **Açıklama:** Sürdürülebilir ekonomik kalkınma, yükselen yaşam standartları, teknolojik ilerleme ve bilginin
    > daha hızlı yayılması, küreselleşmenin **en belirgin faydaları** arasında sayılmaktadır.
    > Toplumsal eşit hakların yaygınlaşması

    <hr />
1. Sürdürülebilirlik kavramı doğrultusunda, atık ya da kullanılmayan nesnenin sanat objesine dönüşme sürecinde, eğitim kurumlarında ders kapsamında öğretilen ve uygulamalarının yapıldığı, temel yöntemler aşağıdakilerden hangisi **değildir?**
    - **Cevap A-) Simülasyon**
    - B-) Dekolaj
    - C-) Kolaj
    - D-) Asamblaj
    - E-) Montaj

    > **Açıklama:** Sürdürülebilirlik kavramı doğrultusunda, atık ya da kullanılmayan nesnenin sanat objesine dönüşme
    > sürecinde, eğitim kurumlarında ders kapsamında öğretilen ve uygulamalarının yapıldığı, temel yöntemler aşağıdaki gibi sıralanabilir.
    > **• Kolaj**
    > **• Dekolaj**
    > **• Asamblaj**
    > **• Akümülasyon**
    > **• Montaj**
    > Simülasyon

    <hr />
1. Elde var olan her türlü basılı, yazılı, çizili, resimsel malzemelerin bir yüzey üzerinde yeniden kurgulanmasıyla oluşturulan temel yöntem aşağıdakilerden hangisidir**?**
    - A-) Montaj
    - **Cevap B-) Kolaj**
    - C-) Asamblaj
    - D-) Akümülasyon
    - E-) Dekolaj

    > **Açıklama:** **Kolaj:** Kelime olarak bir araya getirme birleştirme anlamına gelir. Bir resim tekniğidir. **Elde**
    > **var olan her türlü basılı, yazılı, çizili, resimsel malzemelerin bir yüzey üzerinde yeniden kurgulanmasıyla** oluşturulur.

    <hr />
1. Sökülmek anlamına gelen Fransızca bir kelimedir. Üst üste katmanlar hâlinde yerleştirilmiş afiş, resim ya da basılı kâğıtların bazı bölümlerinin kesilip çıkarılmasıyla oluşturulan teknik aşağıdakilerden hangisidir?
    - A-) Kolaj
    - B-) Montaj
    - C-) Akümülasyon
    - **Cevap D-) Dekolaj**
    - E-) Asamblaj

    > **Açıklama:** **Dekolaj:** “Sökülmek” anlamına gelen Fransızca bir kelimedir. Üst üste katmanlar hâlinde yerleştirilmiş afiş, resim ya da basılı kağıtların bazı bölümlerinin kesilip çıkarılmasıyla oluşturulur.

    <hr />
1. Bir form oluşturacak biçimde kullanılmayan objelerin, birleştirilerek heykele dönüştürülmesi anlamına gelen, üç boyutlu sanat tekniği aşağıdakilerden hangisidir?
    - A-) Kolaj
    - B-) Montaj
    - C-) Akümülasyon
    - D-) Dekolaj
    - **Cevap E-) Asamblaj**

    > **Açıklama:** Bir form oluşturacak biçimde kullanılmayan objelerin birleştirilerek heykele dönüştürülmesi
    > anlamına gelir.
    > Asamblaj

    <hr />
1. Toplumdaki kimlik kaybı ve toplumun bunu fark etmemesi, israfın korkunç boyutlarını Akümülasyon tekniği ile eserlerinde aktaran sanatçı aşağıdakilerden hangisidir?
    - A-) Joshep Beuys
    - B-) Cesar Baldaccini
    - **Cevap C-) Armand Pierre Fernandez**
    - D-) Wolf Vostell
    - E-) Jean Dubuffet

    > **Açıklama:** Fransız sanatçı **Armand Pierre Fernandez** akümülasyon tekniğini kullanan önemli bir sanatçıdır. Eserlerinde “Bugünün Arkeolojisi”ni oluşturmayı önerir. Toplumdaki kimlik kaybı ve toplumun bunu fark etmemesi, israfın korkunç boyutlarını akümülasyon tekniği ile eserlerinde aktarır.

    <hr />
1. Yapmış olduğu projeleriyle ekolojik sanat anlayışının gelişimine katkıda bulunan ilk aktivist sanatçı aşağıdakilerden hangisidir?
    - **Cevap A-) Hans Haacke**
    - B-) Pablo Picasso
    - C-) Cesar Baldacci
    - D-) Jean Dubuffet
    - E-) Wolf Vostell

    > **Açıklama:** **Hans Haacke** ve Joseph Beuys projeleriyle ekolojik sanat anlayışının gelişimine katkıda bulunan
    > ilk aktivist sanatçılardandır.

    <hr />
1. Sürdürülebilir sanat uygulamaları; belirli kriterler sebebiyle desteklenmeli, öğretilmeli ve sürdürülebilir olmalıdır. Aşağıdakilerden hangisi bu kriterlerden **değildir?**
    - A-) Erişilebilir uygun maliyetli malzemelerden yapıldığı için,
    - B-) Siyasi ve sosyal meselelerin altını çizdiği için
    - C-) Yaratıcılığı desteklediği için
    - **Cevap D-) Bilgi ve teknolojiyi kabul etmediği için**
    - E-) Sosyal farkındalık yaratıp, çevre dostu yaklaşımın benimsenmesini sağladığı için

    > **Açıklama:** **Sürdürülebilir sanat uygulamaları;**
    > • Erişilebilir uygun maliyetli malzemelerden yapıldığı için,
    > • Siyasi ve sosyal meselelerin altını çizdiği için,
    > • Yaratıcılığı desteklediği için,
    > • Sosyal farkındalık yaratıp, çevre dostu yaklaşımın benimsenmesini sağladığı için,
    > • Ekolojik dinamikler ve karşılaştığımız çevre sorunları hakkında toplumun bilgilendiren eserler
    > olduğu için desteklenmeli, öğretilmeli ve sürdürülebilir olmalıdır.
    > Bilgi ve teknolojiyi kabul etmediği için

    <hr />
1. Çalışmalarında İleri dönüşüm tekniğini kullanır. Lise döneminden beri atık malzemeleri en yaratıcı biçimde kullanarak heykel yapan sanatçı, kullanılamayacak durumda olan çanta ve valizleri kullanarak tematik Star Wars heykel serisini yaratmıştır. Bu sanatçı aşağıdakilerden hangisidir?
    - A-) David Adjaye
    - **Cevap B-) Gabriel Dishaw**
    - C-) Max Bond
    - D-) Tim Noble
    - E-) Sue Webster

    > **Açıklama:** **Gabriel Dishaw** bu biçimde çalışan sanatçılar için iyi bir örnektir. Çalışmalarında *“Upcycling- İleri*
    > *dönüşüm”* tekniğini kullanan **Gabriel Dishaw**, lise döneminden beri atık malzemeleri en yaratıcı
    > biçimde kullanarak heykel yapan bir sanatçıdır.

    <hr />
## Unite 3
1. Teknolojinin gelişimini, sanayileşmeyi arttıran dönemsel faktörün adı nedir.
    - A-) Karanlık Çağ
    - B-) Edo Dönemi
    - **Cevap C-) Endüstri Devrimi**
    - D-) Endüstri Atağı
    - E-) Rönesans

    > **Açıklama:** Teknolojinin gelişimini, sanayileşmeyi arttıran dönemsel faktörün adı Endüstri Devrimidir, sorunun yanıtı C şıkkıdır.

    <hr />
1. Katı atık hiyerarşisinde ilk tercih edilmesi gereken adımlar hangileridir.
    - A-) Azaltma, Kısma, Tutma
    - B-) Tutma, Biriktirme, Önleme
    - C-) Önleme, Karşılama, Tekrarlama
    - **Cevap D-) Önleme, Azaltma, Tekrar Kullanım**
    - E-) Tekrar Kullanım, Biriktirme, Şekillendirme

    > **Açıklama:** Katı atık hiyerarşisinde ilk tercih edilmesi gereken adımlar Önleme, Azaltma, Tekrar Kullanım dır, sorunun yanıtı D şıkkıdır.

    <hr />
1. Geri dönüşüm kavramı hangi yıl Resmi Gazetede tanımlanmıştır.
    - A-) 1989
    - B-) 2011
    - C-) 2015
    - D-) 2017
    - **Cevap E-) 2019**

    > **Açıklama:** Geri dönüşüm kavramı 2019 yılında Resmi Gazetede tanımlanmıştır, sorunun yanıtı E şıkkıdır.

    <hr />
1. Önleme (prevention):Adımı geri dönüşümün adımları arasında kaçıncı sırada yer alır.
    - A-) 4\.Sırada
    - B-) 3\.Sırada
    - C-) 2\.Sırada
    - **Cevap D-) 1\.Sırada**
    - E-) 5\.Sırada

    > **Açıklama:** Önleme (prevention):Adımı geri dönüşümün adımları arasında 1. sırasında yer alır, sorunun yanıtı D şıkkıdır.
    > 1\.Sırada

    <hr />
1. Bertaraf (disposal) adımı geri dönüşümün adımları arasında kaçıncı sırada yer alır.
    - **Cevap A-) 5\.Sırada**
    - B-) 1\.Sırada
    - C-) 2\.Sırada
    - D-) 4\.Sırada
    - E-) 3\.Sırada

    > **Açıklama:** Bertaraf (disposal) adımı geri dönüşümün adımları arasında 5. sırasında yer alır, sorunun yanıtı A şıkkıdır.
    > 5\.Sırada

    <hr />
1. Geri dönüşüm yoluyla da basılan antik paralara ne ad verilir.
    - A-) Bilye
    - B-) Fiske
    - C-) Muska
    - D-) Mangır
    - **Cevap E-) Sikke**

    > **Açıklama:** Geri dönüşüm yoluyla da basılan antik paralara sikke adı verilir, sorunun yanıtı E şıkkıdır.

    <hr />
1. Tarihte belgelenmiş ilk geri dönüşüm ürünü hangisidir.
    - A-) Zırh
    - B-) Çamaşır
    - C-) Bambu
    - **Cevap D-) Kağıt**
    - E-) Seramik

    > **Açıklama:** Tarihte belgelenmiş ilk geri dönüşüm ürünü kağıttır, sorunun yanıtı D şıkkıdır.

    <hr />
1. 1897 yılında kurulan ilk geri kazanım tesisi hangi şehirde kurulmuştur.
    - A-) Edirne
    - **Cevap B-) New York**
    - C-) Tokyo
    - D-) Roma
    - E-) Vatikan

    > **Açıklama:** 1897 yılında kurulan ilk geri kazanım tesisi New York şehrinde kurulmuştur, sorunun yanıtı B şıkkıdır.

    <hr />
1. Dünya çevre gününün tarihi hangisidir.
    - A-) 14 Haziran
    - B-) 24 Kasım
    - C-) 17 Mart
    - D-) 3 Mart
    - **Cevap E-) 5 Haziran**

    > **Açıklama:** Dünya çevre gününün tarihi 5 hazirandır, sorunun yanıtı E şıkkıdır.

    <hr />
1. Dönen oklardan oluşan geri dönüşüm sembolünü tasarlayan kimdir.
    - A-) Bery White
    - B-) Aydın Erkmen
    - **Cevap C-) Gary Anderson**
    - D-) İhap Hulusi
    - E-) Kenan Mirza

    > **Açıklama:** Dönen oklardan oluşan geri dönüşüm sembolünü tasarlayan Gary Anderson'dur, sorunun yanıtı C şıkkıdır.

    <hr />
1. "Ürünlerin çeşitli yollarla kullanım ömürlerini uzatarak yeniden kullanılması" ifadesi katı atık yönetimi hiyerarşisinde hangi adımı nitelendirmektedir?
    - **Cevap A-) Önleme**
    - B-) Geri dönüşüm
    - C-) Tekrar kullanım
    - D-) Enerji geri kazanımı
    - E-) Bertaraf

    > **Açıklama:** "Önleme (Prevention): Ürünlerin çeşitli yollarla kullanım ömürlerini uzatarak
    > yeniden kullanılmasıdır." Buna göre cevap a seçeneğidir.

    <hr />
1. Tarihte ilk belgelenmiş geri dönüşüm ürünü olan gri kağıt hangi yılda ve hangi ülkede elde edilmiştir?
    - A-) 1031 yılında Amerika'da
    - B-) 1690 yılında Amerika'da
    - C-) 1690 yılında Fransa'da
    - **Cevap D-) 1031 yılında Japonya'da**
    - E-) 1431 yılında İngiltere'de

    > **Açıklama:** "Tarihte ilk belgelenmiş geri dönüşüm ürünü ise 1031 yılında Japonya’da atık kağıtlardan elde edilen gri kağıttır." Buna göre cevap d seçeneğidir.

    <hr />
1. Türkiye'de 2872 sayılı Çevre Kanunu hangi yılda çıkarılmıştır?
    - **Cevap A-) 1983**
    - B-) 1991
    - C-) 2005
    - D-) 2008
    - E-) 2015

    > **Açıklama:** "Türkiye’nin geri dönüşüm açısından atık yönetimi tarihine baktığımızda ise 1983 yılında 2872 sayılı Çevre Kanunu çıkarılmıştır." Buna göre cevap a seçeneğidir.

    <hr />
1. I- Tasarım boyutu<br />II- Espas<br />III- Font türü<br />IV- Mürekkep cinsi<br />Bir grafik tasarım ürününün çevreye etkisini azaltmak için yukarıdaki unsurlardan hangilerine dikkat edilmelidir?
    - A-) Yalnız I
    - B-) I ve II
    - C-) I ve IV
    - D-) I, II, IV
    - **Cevap E-) I, II, III, IV**

    > **Açıklama:** "Bir grafik tasarım ürününün çevreye etkisini azaltmak için, işlev ve estetik boyutu göz ardı edilmeden, kullanılan kağıda, mürekkebe, tasarımın boyutlarına, fontlara, espasa kadar birçok şeye dikkat edilmesi gerekmektedir". Buna göre cevap e seçeneğidir.

    <hr />
1. Evrensel geri dönüşüm sembolüne ilişkin aşağıdaki ifadelerden hangisi **yanlıştır**?
    - A-) Sembolün normal şekilde kullanımı ürünün geri dönüştürülebileceğini belirtir.
    - B-) Sembolün daire içinde kullanımı nesnenin geri dönüştürülmüş malzemeden üretildiğini gösterir.
    - **Cevap C-) Ürün bütünüyle geri dönüştürülmüş malzemeden yapılırsa geri dönüşüm sembolü siyah daire içinde siyah oklarla gösterilmektedir.**
    - D-) Bu sembolün ortasında % sembolüyle yazılmış bir rakam var ise bu da üründe kullanılan geri dönüştürülmüş malzemenin oranını belirtmektedir.
    - E-) Ürün geri dönüştürülmüş malzemenin yanında yeni hammadde kullanılarak üretilmişse beyaz daire içinde siyah geri dönüşüm sembolü ile ifade edilir.

    > **Açıklama:** "Ürün bütünüyle geri dönüştürülmüş malzemeden yapılırsa geri dönüşüm sembolü siyah daire içinde **beyaz** oklarla gösterilmektedir." Buna göre c seçeneğindeki ifade yanlıştır.

    <hr />
1. I- Ampuller<br />II- Floresan lambalar<br />III- Pişirme kapları<br />IV- Araba camları<br />Yukarıdaki cam türlerinden hangileri geri dönüşüm işlemine dâhil edilmez ya da ayrı bir geri dönüşüm süreci gerektirir?
    - A-) Yalnız I
    - B-) I ve II
    - C-) II ve III
    - D-) I, II, III
    - **Cevap E-) I, II, III, IV**

    > **Açıklama:** "....Ancak bazı cam türleri içerdikleri kimyasallar nedeniyle ya geri dönüşüm işlemine dâhil edilmez ya da ayrı bir geri dönüşüm süreci gerektirir. Aynalar, filmli ve optik camlar, ısıya dayanıklı camlar (pişirme kapları vb.), araba ve pencere camları, floresan lambalar ve ampuller bu tür camlardır ve cam atıklar için oluşturulan ünitelere atılmamalıdır" Buna göre cevap e seçeneğidir.

    <hr />
1. Aşağıdakilerden hangisi e-atık olarak nitelendirilmektedir?
    - A-) Ürün ambalajları
    - B-) Piller
    - C-) Motor yağları
    - **Cevap D-) Bilgisayar, telefon vb. ürünler**
    - E-) Tekstil ürünleri

    > **Açıklama:** "Elektrikli veya elektronik atıklar e-atık olarak da kullanılmaktadır. Fırın, bulaşık makinesi vb. beyaz eşyalar, bilgisayar, telefon vb. ürünler bu kategoride yer alır." Buna göre cevap d seçeneğidir.

    <hr />
1. Organik maddelerin daha basit organik ve inorganik maddelere ayrıştırılması işlemine ne ad verilmektedir?
    - A-) İleri dönüşüm
    - B-) Yeniden amaçlandırma
    - **Cevap C-) Kompost**
    - D-) Sıfır atık
    - E-) Kompozit üretim

    > **Açıklama:** "Kompost, organik maddelerin daha basit organik ve inorganik maddelere ayrıştırılması işlemidir". Buna göre cevap c seçeneğidir.

    <hr />
1. İleri dönüşüme ilişkin aşağıdaki ifadelerden hangisi **yanlıştır**?
    - A-) İleri dönüşüm, işlevini yitirmiş kabul edilen her şeyin daha yüksek kaliteye ve estetik değerlere dönüştürülerek yeniden kullanılmasıdır.
    - B-) İleri dönüşüm, ürünlerin atık hâline gelmeden tekrar kullanılmasıyla atık oluşumunu önlemektedir.
    - C-) İleri dönüşümde yukarı yönlü dönüşüm yapılmış olur.
    - **Cevap D-) Gerek kimyasal gerekse mekanik olarak ileri dönüşüm süreci belli miktarda enerji gerektirmektedir**
    - E-) İleri dönüşüm daha az atık depolama alanı kullanılması açısından fayda sağlamaktadır

    > **Açıklama:** "Gerek kimyasal gerekse mekanik olarak geri dönüşüm süreci belli miktarda enerji gerektirmektedir. Ancak ileri dönüşümde buna gerek kalmamaktadır." Bu nedenle cevap d seçeneğidir.

    <hr />
1. I- Beton<br />II- Röntgen filmi<br />III- Motor yağı<br />IV- Bitkisel yağ<br />Yukarıdakilerden hangileri geri dönüştürülebilir atık cinslerinden biridir?
    - A-) Yalnız I
    - B-) I ve II
    - C-) III ve IV
    - D-) I, II, IV
    - **Cevap E-) I, II, III, IV**

    > **Açıklama:** "Beton, ahşap, motor yağı, röntgen filmleri, bitkisel yağların atıkları da geri dönüşütürülebilir atık cinsleridir." Bu nedenle cevap e seçeneğidir.

    <hr />
## Unite 4
1. Eko tasarım konusunda ilk girişimler ABD ve Avrupa’da hangi on yılın sonlarına doğru gerçekleşmiştir?
    - A-) 1960'lar
    - B-) 1970'ler
    - **Cevap C-) 1980'ler**
    - D-) 1990'lar
    - E-) 2000'ler

    > **Açıklama:** Eko tasarım konusunda ilk girişimler ABD ve Avrupa’da 1980’lerin sonlarına doğru gerçekleşmiştir.

    <hr />
1. Aşağıdakilerden hangisinde bir ürünün yaşam döngüsü aşamaları tam olarak verilmiştir?
    - A-) Hammadde ve Malzeme Seçimi Üretim Satış Dağıtım Kullanım Ömrünü Tamamlayan Ürün
    - **Cevap B-) Hammadde ve Malzeme Seçimi Üretim Dağıtım Kullanım Ömrünü Tamamlayan Ürün**
    - C-) Hammadde ve Malzeme Seçimi Planlama Üretim Dağıtım Kullanım Ömrünü Tamamlayan Ürün
    - D-) Hammadde ve Malzeme Seçimi Planlama Tasarım Üretim Dağıtım Kullanım Ömrünü Tamamlayan Ürün
    - E-) Hammadde ve Malzeme Seçimi Üretim Dağıtım Kullanım Geri Dönüşüm

    > **Açıklama:** Hammadde ve Malzeme Seçimi
    > Üretim
    > Dağıtım
    > Kullanım
    > Ömrünü Tamamlayan Ürün

    <hr />
1. Var olan bir firmada, var olan üretim metodu ve ürünün tekrar gözden geçirilmesi üzerine odaklanan eko tasarım yaklaşımı hangisidir?
    - A-) Eko-Kıyaslama
    - B-) Eko-Yenileşim
    - **Cevap C-) Eko-TekrarTasarım**
    - D-) Eko-Sürdürülebilirlik
    - E-) Eko-Geridönüşüm

    > **Açıklama:** EkoTekrarTasarım (EcoReDesign): Var olan bir firmada, var olan üretim metodu ve ürünün tekrar gözden geçirilmesi üzerine odaklanılmasıdır.

    <hr />
1. Bir ürünün üretiminden veya kullanımından doğan çevresel faktörler hakkında bizi bilgilendirmek için oluşturulan tasarımlara ne ad verilir?
    - A-) Eko-markalama
    - **Cevap B-) Eko Etiket**
    - C-) Yeşil PR
    - D-) Eko-Kıyaslama
    - E-) Eko Yenileşim

    > **Açıklama:** Eko etiketler, ürünün üretiminden veya kullanımından doğan çevresel faktörler hakkında bizi bilgilendirmek için tasarlanır.

    <hr />
1. SAGE aşağıdakilerden hangisinin İngilizce'sinin kısaltmasıdır?
    - A-) Yeşil Etiket Derneği
    - **Cevap B-) Çevresel Stratejik Danışma Grubu**
    - C-) Eko Etiket Tasarım Enstitüsü
    - D-) Dünya Bankası Çevre Gurubu
    - E-) Kyoto Sözleimesi

    > **Açıklama:** SAGE (The Strategic Advisory Group on Environmental/Çevresel Stratejik Danışma Grubu)

    <hr />
1. Dünyada ürün ve hizmetleri çevre konusunda değerlendiren ilk çevre etiketi hangisidir?
    - A-) Energy Star
    - B-) Eco-Mark
    - C-) EU Ecolabel
    - **Cevap D-) Blue Angel**
    - E-) WaterSense

    > **Açıklama:** Almanya merkezli Blue Angel, dünyada ürün ve hizmetleri çevre konusunda değerlendiren ilk çevre etiketidir.

    <hr />
1. Aşağıdakilerden hangisi "Tip II Eko Etiketi"dir?
    - **Cevap A-) Energy Star**
    - B-) Blue Angel
    - C-) Eco-Mark
    - D-) Global GreenTag Certified
    - E-) Çevre Etiketi

    > **Açıklama:** Eco-Mark programı tüketiciler için daha bilinçli seçimler yaparak doğaya daha dost bir yaşam tarzı oluşturmaya hizmet eden Tip I Eko Etiketidir.
    > **Tip II eko etiket** kapsamında olan, **Energy Star** (ABD), WaterSense (ABD), SCS Recycled Content (ABD)
    > gibi etiketlerin tanımları aşağıda verilmiştir;
    > **Tip I eko etiket** kapsamında olan, **Blue Angel** (Almanya), EU Ecolabel (Avrupa Birliği), **Eco-Mark** (Japonya) ve **Çevre Etiketi** (Türkiye) bazı etiket örnekleri aşağıda verilmiştir;
    > **Tip III** **eko etiket** kapsamında olan, **Global GreenTag**
    > **Certified (Avustralya),** Climatop (İsviçre) gibi etiketlerin tanımları aşağıda verilmiştir;

    <hr />
1. Eco-Mark etiketini hangi ülke öne sürmüştür?
    - A-) Almanya
    - B-) A.B.D.
    - **Cevap C-) Japonya**
    - D-) Danimarka
    - E-) İsveç

    > **Açıklama:** Eco-Mark (Japonya): Eco-Mark programı tüketiciler için daha bilinçli seçimler yaparak doğaya daha dost bir yaşam tarzı oluşturmaya hizmet eder.

    <hr />
1. Global GreenTag Certified etiketini hangi ülke öne sürmüştür?
    - A-) Almanya
    - **Cevap B-) Avusturalya**
    - C-) Japoınya
    - D-) İsveç
    - E-) Danimarka

    > **Açıklama:** Global GreenTag Certified (Avustralya): Global GreenTag Certified® bilimsel ve yaşam döngüsü değerlendirmesi süreciyle desteklenen bir üçüncü taraf yeşil ürün değerlendirme ve belgelendirme sistemidir

    <hr />
1. Climatop etiketini hangi ülke öne sürmüştür?
    - A-) İsveç
    - **Cevap B-) İsviçre**
    - C-) Finlandiya
    - D-) Danimarka
    - E-) İslanda

    > **Açıklama:** Climatop (İsviçre): Climatop’ın amacı kendi sınıflarında iklime en dost ürün ve hizmetleri seçmektir.

    <hr />
1. Doğa bilimlerinde organizmayla çevresi arasındaki ilişkileri inceleyen bilim dalına ne denir?
    - **Cevap A-) Ekoloji**
    - B-) Eko Tasarım
    - C-) Üretim
    - D-) Hammadde
    - E-) Geri dönüşüm

    > **Açıklama:** Ekoloji, doğa bilimlerinde organizmayla çevresi arasındaki ilişkileri inceleyen bilim dalıdır.

    <hr />
1. Aşağıdakilerden hangisi günümüz ekonomisi hangi sloganı ile bizi, öngörülemez ve karanlık bir geleceğe doğru sürüklemektedir?
    - **Cevap A-) Satın al kullan at**
    - B-) Ekolojik çevre
    - C-) Çevre için tasarım
    - D-) Eco tasarım
    - E-) Sürdürülebilirlik için Tasarım

    > **Açıklama:** ...günümüz ekonomisi, “satın al, kullan, at” sloganı ile bizi, öngörülemez ve karanlık bir geleceğe doğru sürüklemektedir.

    <hr />
1. 1990 yılının başlarında “söz” adını verdikleri ilk eko tasarım kılavuzunu hangi ülke çıkartmıştır?
    - **Cevap A-) Hollanda**
    - B-) Kosta Rika
    - C-) Norveç
    - D-) Belçika
    - E-) Kolombiya

    > **Açıklama:** 1990’ların başında Hollanda’da 8 farklı sektörle (mobilya, otomotiv, ambalaj vb.) yapılan örnek proje ilk ciddi eko tasarım girişimi olmuştur. 1994’te Delft Teknoloji üniversitesi araştırma programı “Sürdürülebilirlik için Tasarım” diğer Hollandalı ortakları ile “söz” adını verdikleri ilk eko tasarım kılavuzunu çıkartmışlardır.

    <hr />
1. Eko tasarım konusunda ilk girişimler ABD ve Avrupa’da hangi tarihte gerçekleşmiştir?
    - **Cevap A-) 1980’lerin sonları**
    - B-) 1981’lerin sonları
    - C-) 1982’lerin sonları
    - D-) 1979’lerin sonları
    - E-) 1983’lerin sonları

    > **Açıklama:** Eko tasarım konusunda ilk girişimler ABD ve Avrupa’da 1980’lerin sonlarına doğru gerçekleşmiştir.

    <hr />
1. Planlanan ürünün üretilmesi için gerekli olan hammaddenin elde edilmesinden, fonksiyonel bir ürüne dönüşmesine ve kullanım ömrü sonrasına kadar tüm yaşam döngüsünün tasarlanmasına ne denir?
    - **Cevap A-) Eko tasarım**
    - B-) Çevre Tasarım
    - C-) Yeşil Mühendislik
    - D-) Çevreye Duyarlı Tasarım
    - E-) Beşikten Beşiğe Tasarım

    > **Açıklama:** Eko tasarım, planlanan ürünün üretilmesi için gerekli olan hammaddenin elde edilmesinden, fonksiyonel bir ürüne dönüşmesine ve kullanım ömrü sonrasına kadar tüm yaşam döngüsünün tasarlanması olarak tanımlanabilir

    <hr />
1. Aşağıdakilerden hangisi Yasam Döngüsü Asamalarından biri **değildir?**
    - **Cevap A-) Enerji**
    - B-) Hammadde ve malzeme seçimi
    - C-) Üretim
    - D-) Dagıtım
    - E-) Ömrünü tamamlayan ürün

    > **Açıklama:** **Yasam Döngüsü Asamaları**
    > Hammadde ve malzeme seçimi
    > Üretim
    > Dagıtım
    > Kullanım
    > Ömrünü tamamlayan ürün

    <hr />
1. Aşağıdakilerden hangisi *eko tasarım* kavramını ayırt edebilmemizde yol gösterici olacak belirleyici özelliklerdendir?<br />I. Daha iyi tasarım ile ürünün tüm yaşam döngüsü boyunca çevresel etkilerin azaltılmasıdır.<br />II. Ürüne ait kalite, fiyat, güvenlik gibi faktörlerle birlikte çevresel etkisinin de hesaplanmasıdır.<br />III. Üretilecek olan ürünün henüz hammadde aşamasındayken çevreye etkisinin araştırılması ve üretim sürecinin bu şekilde planlanmasıdır.<br />IV. Sosyal ve kültürel yönden kabul görebilecek yeni önerilerin, teknik olarak mümkün, çevre için mecburi olanlarını birleştiren bir tasarım faaliyetidir.
    - **Cevap A-) I/II/III/IV**
    - B-) II/III/IV
    - C-) I/II/III
    - D-) I/II/IV
    - E-) I/III/IV

    > **Açıklama:** Sonuç olarak eko tasarımın ve birbirleri yerine kullanılan diğer kavramların ortak amacının çevreyi korumak olmasına karşın, kapsam olarak birbirlerinden farklılıkları vardır.Bu nedenle aşağıda *eko tasarım* kavramını ayırt edebilmemizde yol gösterici olacak belirleyici bazı özellikler yer almaktadır:
    > • Daha iyi tasarım ile ürünün tüm yaşam döngüsü boyunca çevresel etkilerin azaltılmasıdır.
    > • Ürüne ait kalite, fiyat, güvenlik gibi faktörlerle birlikte çevresel etkisinin de hesaplanmasıdır.
    > • Üretilecek olan ürünün henüz hammadde aşamasındayken çevreye etkisinin araştırılması ve üretim sürecinin bu şekilde planlanmasıdır.
    > • Sosyal ve kültürel yönden kabul görebilecek yeni önerilerin, teknik olarak mümkün, çevre için mecburi olanlarını birleştiren bir tasarım faaliyetidir.

    <hr />
1. Temelde ekoloji ve ekolojik sistemlerin fonksiyonlarını, süreçlerini ve üretkenliğini gelecekte de devam ettirebilme yeteneği olarak algılanan kavram aşağıdakilerden hangisidir?
    - **Cevap A-) Sürdürülebilirlik**
    - B-) Sosyal çevre
    - C-) Çevresel ekonomi
    - D-) Tasarım
    - E-) Ekolojik sistem

    > **Açıklama:** Kısaca tanımlamak gerekirse ***“Sürdürülebilirlik,*** temelde ekoloji ve ekolojik sistemlerin fonksiyonlarını, süreçlerini ve üretkenliğini gelecekte de devam ettirebilme yeteneği olarak algılanmaktadır.”

    <hr />
1. Aşağıdakilerden hangisi veya hangileri 1980’lerin başında, ünlü ürün ve mobilya tasarımcısı Dieter Rams kendine “Tasarım iyi bir tasarım mı?” sorusunun ilkeleridir?<br />I. İyi tasarım yenilikçidir.<br />II. İyi tasarım bir ürünü kullanışlı kılar.<br />III. İyi tasarım estetiktir.<br />IV. İyi tasarım bir ürünü anlaşılır kılar.<br />V. İyi tasarım göze batmaz.<br />VI. İyi tasarım dürüsttür.<br />VII. İyi tasarım uzun ömürlüdür.<br />VIII. İyi tasarım en ince ayrıntısına kadar eksiksizdir.<br />IX. İyi tasarım çevre dostudur.<br />X. İyi tasarım mümkün olduğunca az tasarımdır
    - **Cevap A-) I/II/III/IV/V/VI/VII/VIII/IX/X**
    - B-) I/II/III/IV/V/VII/VIII/IX/X
    - C-) I/II/IV/V/VI/VII/VIII/IX/X
    - D-) II/III/IV/V/VI/VII/VIII/IX/X
    - E-) I/II/III/IV/V/VI/VII/VIII/IX/

    > **Açıklama:** Dieter Rams’ın iyi tasarım için on ilkesi aşağıdaki gibidir;
    > 1980’lerin başında, ünlü ürün ve mobilya tasarımcısı Dieter Rams kendine “Tasarım iyi bir tasarım mı?” sorusunu sordu.
    > 1\. İyi tasarım yenilikçidir.
    > 2\. İyi tasarım bir ürünü kullanışlı kılar.
    > 3\. İyi tasarım estetiktir.
    > 4\. İyi tasarım bir ürünü anlaşılır kılar.
    > 5\. İyi tasarım göze batmaz.
    > 6\. İyi tasarım dürüsttür.
    > 7\. İyi tasarım uzun ömürlüdür.
    > 8\. İyi tasarım en ince ayrıntısına kadar eksiksizdir.
    > 9\. İyi tasarım çevre dostudur.
    > 10\. İyi tasarım mümkün olduğunca az tasarımdır

    <hr />
1. Attığı her adımda ticari faaliyetleri için ihtiyaç duyduğu kaynakların gelecek nesillere azalmadan, tam tersine geliştirilerek aktarılmasına özen gösterme çabasında olup, bilinçli üretim-bilinçli tüketim döngüsü ile daha yaşanır bir gelecek için çalışmalarını sürdüren şirket hangisidir?
    - **Cevap A-) Unilever**
    - B-) Procter & Gamble
    - C-) Colgate-Palmolive
    - D-) Henkel
    - E-) Amway

    > **Açıklama:** **Unilever**
    > Unilever Türkiye, attığı her adımda ticari faaliyetleri için ihtiyaç duyduğu kaynakların gelecek nesillere azalmadan, tam tersine geliştirilerek aktarılmasına özen gösterme çabasında olup, bilinçli üretim-bilinçli tüketim döngüsü ile daha yaşanır bir gelecek için çalışmalarını sürdürmektedir.

    <hr />
## Unite 5
1. Grafik tasarımcıların mesleki sorumlulukları ve tasarım önceliklerini vurgulayan "Önce Öncelikler" manifestosu, 2014 yılında kaç tasarımcının imzası ile güncellenmiştir?
    - A-) 20
    - B-) 220
    - C-) 1320
    - D-) 1500
    - **Cevap E-) 1600**

    > **Açıklama:** Grafik tasarımcıların mesleki sorumlulukları ve tasarım öncelikleri ilk olarak 1963 yılında Ken Garland tarafından kaleme alınan ve 20 tasarımcının altına imza attığı, “Önce Öncelikler” (First Things First) manifestosu ile vurgulanmıştır. 1999 yılında yeniden kaleme alınan ve 2014 yılında 1600 tasarımcının imzası ile güncellenen manifesto, 2020 yılında güncellenmiş, çok sayıda dile çevrilerek, grafik tasarımcılara önceliklerini sorgulamayı hatırlatmıştır. Önce Öncelikler 2020 manifestosu grafik tasarımcılara doğadan bağımız olmadıklarını ve eylemlerini bu bilinçle gerçekleştirmeleri gerektiğini salık verirken, tasarımlarıyla Dünya ve onun üzerindeki canlı yaşam arasındaki ilişkiyi yeniden tesis etmeyi amaçlamalarını, yeteneklerini insanlığın daha ekolojik bir medeniyet yolunda iyileştirilmesi için kullanmalarını söyler.
    > Grafik tasarımcıların mesleki sorumlulukları ve tasarım öncelikleri ilk olarak 1963 yılında Ken Garland tarafından kaleme alınan ve 20 tasarımcının altına imza attığı, “Önce Öncelikler” (First Things First) manifestosu ile vurgulanmıştır. 1999 yılında yeniden kaleme alınan ve 2014 yılında 1600 tasarımcının imzası ile güncellenmiştir.

    <hr />
1. Aşağıdaki kavramlardan hangisi grafik tasarımda bir grafik tasarım ürününün (afiş, ambalaj, katalog, kitap kapağı vb.) uzunluk, genişlik ve derinliğinin boyutlarını sayısal olarak niteleyen değerdir?
    - **Cevap A-) Ölçü**
    - B-) Tipografi
    - C-) Ofset
    - D-) Manifesto
    - E-) PNG

    > **Açıklama:** Grafik tasarımda ölçü, isminden de anlaşıldığı gibi, bir grafik tasarım ürününün (afiş, ambalaj, katalog, kitap kapağı vb.) uzunluk, genişlik ve derinliğinin boyutlarını sayısal olarak niteleyen değerdir. Bu değer, grafik ürünün türüne göre yalnızca iki boyutlu (en ve boy) ya da üç boyutlu (en, boy, derinlik) olabilir.

    <hr />
1. Mürekkebin kâğıda ya da belli bir yüzeye uygulanmasına yarayan çeşitli yöntemleri işaret eden genel kavram aşağıdakilerden hangisidir?
    - **Cevap A-) Baskı**
    - B-) Ofset
    - C-) Serigrafi
    - D-) Gravür
    - E-) Tipo

    > **Açıklama:** Baskı kavramı, mürekkebin kâğıda ya da belli bir yüzeye uygulanmasına yarayan çeşitli yöntemleri işaret eden, genel bir kavramdır. Ofset baskı, serigrafi, gravür, tipo baskı, püskürtmeli yazıcılar ya da lazer baskı, söz konusu baskı türlerinden yalnızca bazılarıdır.

    <hr />
1. Siyah beyaz lazer yazıcı kâğıt üzerinde standart bir okunurluk sunarken, aşağıdakilerden hangisini sağlayamaz?
    - **Cevap A-) Harflerin tipo baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisi**
    - B-) Harflerin gravür baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisi
    - C-) Harflerin PNG baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisi
    - D-) Harflerin ofset baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisi
    - E-) Harflerin serigrafi baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisi

    > **Açıklama:** Baskı kavramı, mürekkebin kâğıda ya da belli bir yüzeye uygulanmasına yarayan çeşitli yöntemleri işaret eden, genel bir kavramdır. Ofset baskı, serigrafi, gravür, tipo baskı, püskürtmeli yazıcılar ya da lazer baskı, söz konusu baskı türlerinden yalnızca bazılarıdır. Her baskı türünün kendi içinde farklı değişenleri bulunur. Baskı hızı, renk çeşitliliği, baskı kapasitesi veya maliyeti, seçilen baskı yöntemine göre değişkenlik gösterir. Ayrıca farklı baskı yöntemleri kâğıt ya da yüzey üzerinde farklı etkilere sahiptir. Siyah beyaz lazer yazıcı kâğıt üzerinde standart bir okunurluk sunarken, harflerin tipo baskı sırasında kâğıt üzerinde oluşan çukurlaşmış etkisini sağlayamaz.

    <hr />
1. Temelde su ve yağın birbirine karışmaması prensibine dayanan geleneksel baskı yöntemi aşağıdakilerden hangisidir?
    - **Cevap A-) Ofset litografi**
    - B-) Ofset pitografi
    - C-) Fleksografi
    - D-) Serigrafi
    - E-) Baskıgrafi

    > **Açıklama:** Ofset litografi: Ofset baskı olarak da bilinen bu baskı yöntemi, temelde su ve yağın birbirine karışmaması prensibine dayanmaktadır. Mürekkep verilmiş bir klişenin (kalıbın) merdanelerden faydalanan bir sistem aracılığıyla kauçuk bir yüzeye, buradan da kâğıda aktarılması olarak özetlenebilir.

    <hr />
1. Fleksografi ismini aşağıdakilerden hangisinden almıştır?
    - **Cevap A-) Esnek kalıplar**
    - B-) Tipo kalıplar
    - C-) Serigrafi kalıplar
    - D-) PNG kalıplar
    - E-) Ofset kalıplar

    > **Açıklama:** Flekso baskı olarak da bilinen fleksografi de temel çalışma prensibi bakımından tipo baskı yöntemine benzeyen bir baskı çeşididir. İngilizce esnek anlamına gelen “flex” sözcüğünden türetilmiş olan fleksografi, esnek bir kalıp kullanır. Tıpkı tipo baskı gibi yükseltilmiş bir baskı yüzeyi kullanan bu baskı yöntemi, tipo baskıdan farklı olarak çok yüksek hızlı bir döner çalışma sistemine sahiptir. Plastikten metal filmlere kadar çok çeşitli yüzeye baskı yapma imkânı verdiği için ambalaj baskılarında en çok kullanılan yöntemlerin başında gelir.
    > Fleksografi ismini kullanılan esnek kalıplardan almıştır.

    <hr />
1. Farklı sayfaları tek bir ürün (kitap, dergi, katalog vb.) olacak şekilde, sağlam biçimde bir araya getirmeyi tanımlayan kavram aşağıdakilerden hangisidir?
    - **Cevap A-) Ciltleme**
    - B-) Ofset
    - C-) Serigrafi
    - D-) Litografi
    - E-) Baskı

    > **Açıklama:** Ciltleme farklı sayfaları tek bir ürün (kitap, dergi, katalog vb.) olacak şekilde, sağlam biçimde bir araya getirmektir. Ciltlemenin çok çeşitli şekil ve yöntemleri vardır. Bu yöntemlerin her birinin maliyeti, görsel sonucu, ideal sayfa sayısı aralığı, işlevi ya da kullanım süresi diğerlerinden farklıdır ve ciltleme türünün seçiminde bu unsurların hesaba katılması gerekir.

    <hr />
1. Bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanan grafik türü aşağıdakilerden hangisidir?
    - **Cevap A-) Vektörel**
    - B-) Raster
    - C-) Tipo
    - D-) Serigrafi
    - E-) PNG

    > **Açıklama:** Vektörel grafik: Vektör dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Bir vektör dosyasının matematiksel formülleri görüntü oluşturmak için şekil, kenarlık ve dolgu rengini tanımlar. Bu formül her boyutta yeniden hesaplanabildiğinden vektör görüntülerini kaliteden ödün vermeden sınırsız şekilde büyütmek ya da küçültmek mümkündür.

    <hr />
1. Bilgigrafi kavramının diğer adı aşağıdakilerden hangisidir?
    - **Cevap A-) İnfografik**
    - B-) Tipografik
    - C-) Serigrafik
    - D-) PNGfik
    - E-) Sürdürülebilirlik

    > **Açıklama:** Bilgigrafi (infografik) tasarımları, grafik tasarımın en etkili görsel iletişim kurma araçlarından biridir. Bu grafikler, büyük miktarlardaki verileri ya da karmaşık bilgileri hızlı ve kolay anlaşılabilir biçimde görselleştirmeye yarar. Etkili biçimde kullanıldığında yazıyla paylaşıldığında çok uzun bir anlatım gerektirecek bir bilgiyi çok daha küçük bir alan kullanarak görselleştirmeye ve okuyucuya iletmeye yardımcı olurlar.

    <hr />
1. Aşağıdakilerden hangisi sürdürülebilir grafik tasarım için alınabilecek kararlardan biri olamaz?
    - A-) Baskı alanını etkili şekilde kullanmak
    - **Cevap B-) Kalın ağırlığa sahip (bold) yazı karakterleri kullanmak**
    - C-) Çevre dostu özel yazı karakterleri kullanmak
    - D-) Zeminde açık renkler kullanmak
    - E-) Yazıcı dostu tasarımlar yapmak

    > **Açıklama:** Tasarımda kullanılan tipografik elemanların seçim ve yerleşimi söz konusu tasarımı daha çevre dostu kılabilir. Örneği kalın ağırlığa sahip (bold) yazı karakterleri daha fazla mürekkep harcanmasına sebep olacaktır. Kullanılan yazı karakteri tasarımının da sürdürülebilirlik konusunda etkileri olabileceği unutulmamalıdır. Tasarımı gereği okunurluğu düşük olan yazı karakterlerini daha okunur kılmak için yazı boyutunu büyütmek gerekebilir. Bu da metnin tasarım alanında gereğinden fazla yer kaplamasına ve tıpkı sayfa düzeninde öğelerin hatalı yerleşiminde olduğu gibi fazladan sayfa sayısına ve israfa neden olabilir.
    > Kalın ağırlığa sahip (bold) yazı karakterleri daha fazla mürekkep harcanmasına sebep olur.

    <hr />
1. Grafik tasarımcıların mesleki sorumlulukları ve tasarım öncelikleri ilk olarak hangi yılda kim tarafından nasıl vurgulanmıştır?
    - **Cevap A-) 1963 yılında Ken Garland tarafından ve 20 tasarımcının altına imza attığı, “Önce Öncelikler” (First Things First) manifestosu ile vurgulanmıştır.**
    - B-) 1999 yılında Ambrose ve Harris tarafından yazılmıştır.
    - C-) 2014 yılında 1600 tasarımcının imzası ile W.Jedlicka tarafından yayınlanan manifesto ile.
    - D-) Johansson, Lundberg, ve Ryberg tarafından 2012 yılında.
    - E-) C.F. Barth tarafından 2008 yılında.

    > **Açıklama:** Grafik tasarımcıların mesleki sorumlulukları ve tasarım öncelikleri ilk olarak 1963 yılında Ken Garland tarafından kaleme alınan ve 20 tasarımcının
    > altına imza attığı, “Önce Öncelikler” (First Things First) manifestosu ile vurgulanmıştır.
    > 1963 yılında Ken Garland tarafından ve 20 tasarımcının altına imza attığı, “Önce Öncelikler” (First Things First) manifestosu ile vurgulanmıştır.

    <hr />
1. Aşağıdaki şıklardan hangisi ticari amaçlı başlıca baskı yöntemlerinden birisidir?
    - A-) Fotokopi makinası.
    - **Cevap B-) Fleksografi.**
    - C-) Lazer yazıcı.
    - D-) İnfekt
    - E-) Dijital baskı

    > **Açıklama:** Ticari amaçlı başlıca baskı yöntemleri, ofset litografi, tipo baskı, fleksografi ve serigrafidir. Bu yöntemlerin bazıları ticari olmayan amaçlarla ya da toplu üretim amacı taşımayan özgün baskı üretimi amacıyla da
    > kullanılmaktadır.
    > Fleksografi.

    <hr />
1. Geleneksel bir görünüm istenen kartvizit ya da davetiye basımında ya da kabartma, çukurlaştırma, perfore gibi baskı sonlandırma işlemleri için kullanılan baskı yöntemi aşağıdakilerden hangisidir?
    - A-) Dijital baskı
    - B-) Ofset baskı
    - **Cevap C-) Tipo baskı**
    - D-) Serigrafi baskı
    - E-) Fotokopi

    > **Açıklama:** Tipo Baskı ve Fleksografi: Geçmişi Orta Çağlardaki ahşap baskılara ve Gutenberg’in icadı olan baskı
    > makinesine kadar dayanan tipo baskı, klasik baskı yöntemlerinden biridir. Ofset baskının aksine, bu kez
    > mürekkep taşıyıcı yüzeyin baskı oluşturacak kısımları, baskının oluşmayacağı kısımlardan yüksektedir. Bu açıdan bakıldığında, hepimizin bildiği mühürlerin çalışma şekline benzer (Johansson, Lundberg, ve
    > Ryberg, 2012). Yüksek sayıdaki ticari baskı ihtiyaçları için çok elverişli olmayan tipo baskı yöntemi daha geleneksel bir görünüm arzu edilen kartvizit ya da davetiye gibi baskılarda, numaralandırma gerektiren form ve fatura benzeri belgelerin basımında ya da kabartma, çukurlaştırma, perfore gibi baskı sonlandırma işlemleri için kullanılmaktadır.

    <hr />
1. Aşağıdaki şıklardan hangisi "serigrafi baskının çalışma yöntemini" açıklar?
    - A-) Esnek bir kalıp kullanılır. Tıpkı tipo baskı gibi yükseltilmiş bir baskı yüzeyi kullanan bu baskı yöntemi, çok yüksek hızlı bir döner çalışma sistemine sahiptir.
    - B-) Su ve yağın birbirine karışmaması prensibine dayanmaktadır. Mürekkep verilmiş bir kalıbın merdanelerden faydalanan bir sistem aracılığıyla kauçuk bir yüzeye, buradan da kâğıda aktarılması olarak özetlenebilir.
    - C-) Lazer yazıcılarda ve fotokopi makinelerinde kullanılan baskı yöntemidir.
    - **Cevap D-) Kıvamlı bir mürekkebin, yönteme adını da veren ipek gibi gözenekli bir kalıptan geçerek, baskı yüzeyine aktarılması olarak özetlenebilir.**
    - E-) Elektriksel yüklenme kullanır. Baskı işlemi esnasında yoğunlaştırılmış ışınlar, silindirik bir tambura yansıtılır. Belli bir elektrik yüküne sahip olan bu tambur, lazer ışınlarının vurduğu alanlarda tersine bir elektrik yüküne sahip olur. Basılacak imaj, elektriksel biçimde oluşturulmuş olur. Elektriksel yüke sahip bir mürekkep olan toneri kendine çeker. Çekilen toner kâğıda aktarılır.

    > **Açıklama:** Serigrafi baskının çalışma yöntemi, temelde kıvamlı bir mürekkebin, yönteme adını da veren ipek gibi
    > gözenekli bir kalıptan geçerek, baskı yüzeyine aktarılması olarak özetlenebilir. Çok ince ve küçük gözenekli bir kumaş, çerçeveye gerilir. Her farklı baskı ve kimi zaman her bir farklı renk için ayrı ayrı
    > hazırlanan bu çerçeveye, ışığa duyarlı emülsiyon yardımıyla baskı için kullanılacak imaj pozlandırılır. Pozlandırmanın ardından yapılan yıkama işlemi sonucunda baskı oluşturmayacak alanlardaki gözenekler emülsiyonun sertleşmesi ile tıkanır, baskı elde edilecek gözeneklerdeki emülsiyon akıp gider. Böylece baskıda kullanılacak kalıp hazırlanmış olur.

    <hr />
1. Baskı için kullanılan kağıtların kabul edilmiş çeşitli boyut standartları bulunur. En yaygın kullanılanları ISO tarafından belirlenmiş olan ISO 216 standartlarıdır. Bu doğrultuda kağıt ve zarflar için tanımlanmış ev ve ofis tipi yazıcılarda sıklıkla kullanılan kağıtların ölçüleri aşağıdakilerden hangisidir?
    - A-) 70x100 cm olmak üzere, 64x90 cm, 57x82 cm, 59x84 cm gibi çeşitli standart tabaka ölçülerine sıkça rastlanır.
    - B-) A ve B serileri, A0, A1, A2, …, A10 şeklinde ilerleyen bu serilerde her kâğıt bir öncekinin yarı alanına sahiptir. Sayı küçüldükçe kâğıt ölçüsü küçülür.
    - C-) C serisi tanımlanmıştır. Her kâğıt bir öncekinin yarı alanına sahiptir. Sayı küçüldükçe kâğıt ölçüsü büyür.
    - D-) B serisi, her kâğıt bir öncekinin yarı alanına sahiptir. Sayı küçüldükçe kâğıt ölçüsü küçülür.
    - **Cevap E-) A ve B serileri kağıtlar için, C serisi ise zarflar için tanımlanmıştır. A0, A1, A2, …, A10 şeklinde ilerleyen bu serilerde her kâğıt bir öncekinin yarı alanına sahiptir. Sayı küçüldükçe kâğıt ölçüsü büyür.**

    > **Açıklama:** Baskı için kullanılan kağıtların dünya genelinde kabul edilmiş çeşitli boyut standartları bulunur. Bunlardan en yaygın kullanılanları ISO (International Organization for Standardization - Uluslararası Standartlar Teşkilatı) tarafından belirlenmiş olan ISO 216 standartlarıdır (Johansson, Lundberg ve Ryberg, 2012). A ve B serileri kağıtlar için, C serisi ise zarflar için tanımlanmıştır. A0, A1, A2, …, A10 şeklinde ilerleyen bu serilerde her kâğıt bir öncekinin yarı alanına sahiptir. Sayı küçüldükçe kâğıt ölçüsü büyür. Ev ve ofis tipi yazıcılarda ve fotokopi makinelerinde sıklıkla kullanılan A4 kağıtlar bu standart serinin en bilinen örneklerindendir.
    > A ve B serileri kağıtlar için, C serisi ise zarflar için tanımlanmıştır. A0, A1, A2, …, A10 şeklinde ilerleyen bu serilerde her kâğıt bir öncekinin yarı alanına sahiptir. Sayı
    > küçüldükçe kâğıt ölçüsü büyür

    <hr />
1. Kâğıt üzerinde fiziksel basınç ile oluşturulan kabartma etkisine ne ad verilir?
    - **Cevap A-) Gofre**
    - B-) Selefon kaplama
    - C-) Fire
    - D-) Ciltleme
    - E-) İnkjet

    > **Açıklama:** Gofre: Kâğıt üzerinde fiziksel basınç ile oluşturulan kabartma etkisi (Uçar, 2019)

    <hr />
1. Aşağıdaki şıklardan hangisi "Vektörel grafik" ifadesini tanımlar?
    - A-) Zemin rengi sürdürülebilirliği etkileyen kararlardandır. Zeminin tamamının doygun bir renkle renklendirilmesi daha fazla mürekkep kullanımı anlamına gelirken zeminde açık renkler kullanmak ya da beyaz boşluktan akıllıca faydalanmak, mürekkep tüketimini azaltacaktır.
    - **Cevap B-) Bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Dosya matematiksel formülleri görüntü oluşturmak için şekil, kenarlık ve dolgu rengini tanımlar.**
    - C-) Harflerin ve kelimelerin nasıl düzenlendiğini, birbirleriyle ilişkili olarak nasıl oluşturulduklarını ve bir kompozisyon içinde işgal ettikleri yeri içerir.
    - D-) Piksellerden oluşturulan görüntülerdir. Pikseller bir araya gelerek fotoğraf gibi çok ayrıntılı görüntüleri oluşturabilen, çok büyük miktarda küçük renkli karelerdir.
    - E-) Dijital bir grafik dosyasının bilgisayar belleğinde kapladığı yer arttıkça bu dosyanın üretimi, işlenmesi ve görüntülenmesi sırasında harcanacak enerjinin asgari olması.

    > **Açıklama:** Vektörel grafik: Vektör dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler,
    > çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Bir vektör dosyasının matematiksel formülleri
    > görüntü oluşturmak için şekil, kenarlık ve dolgu rengini tanımlar. Bu formül her boyutta yeniden hesaplanabildiğinden vektör görüntülerini kaliteden ödün vermeden sınırsız şekilde büyütmek ya da küçültmek mümkündür (Adobe, 2022b).
    > Bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Dosya matematiksel formülleri görüntü oluşturmak için şekil, kenarlık ve dolgu rengini tanımlar.

    <hr />
1. Vektörel grafik ile Raster grafik arasındaki fark nedir?
    - A-) Vektör dosyaları, harflerin ve kelimelerin nasıl düzenlendiğini, birbirleriyle ilişkili olarak nasıl oluşturulduklarını ifade eder. Raster grafik dosyaları, tamamen görselleri içerir.
    - B-) Vektör dosyaları,düşük sayıda kopyalar için idealdir. Raster grafik dosyaları, daha yüksek kalitede baskı sunar.
    - **Cevap C-) Vektör dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Raster grafik dosyaları, piksellerden oluşturulan görüntülerdir.**
    - D-) Vektör dosyaları, düşük çözünürlüğü olan dosyalardır. Raster grafik dosyaları, çok yüksek çözünürlükteki dosyaları içerir.
    - E-) Raster grafik dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Vektör dosyaları, piksellerden oluşturulan görüntülerdir.

    > **Açıklama:** Vektörel grafik: Vektör dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler,
    > çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Bir vektör dosyasının matematiksel formülleri
    > görüntü oluşturmak için şekil, kenarlık ve dolgu rengini tanımlar. Bu formül her boyutta yeniden hesaplanabildiğinden vektör görüntülerini kaliteden ödün vermeden sınırsız şekilde büyütmek ya da küçültmek mümkündür (Adobe, 2022b).
    > Raster grafik: Raster grafik dosyaları, piksellerden oluşturulan görüntülerdir. Pikseller bir araya gelerek fotoğraf gibi çok ayrıntılı görüntüleri oluşturabilen, çok büyük miktarda küçük renkli karelerdir. Bir görüntünün piksel sayısı ne kadar fazla olursa görüntü kalitesi ve dosya boyutu da o kadar yüksek olur (Adobe, 2022b)
    > Vektör dosyaları, bir görüntü oluşturmak için ızgara üzerinde matematiksel denklemler, çizgiler ve eğriler kullanır. Bu dosyalarda piksel yoktur. Raster grafik dosyaları, piksellerden oluşturulan görüntülerdir.

    <hr />
1. Aşağıdaki şıklardan hangisi iklim değişikliği konusunda çalışmalar yapan My Climate organizasyonu dijital karbon ayak izini azaltmak için tüm bilgisayar ve internet kullanıcılarına önerdiklerinden birisi değildir?
    - A-) Cihazları daha uzun süre kullanmak.
    - B-) Kullanılmayan elektronik cihazları doğru şekilde geri dönüşüme kazandırmak.
    - **Cevap C-) E-posta kutularını her zaman dolu tutmak. Verileri masa üzerine kayıt etmemek.**
    - D-) Dosyaları mümkün olduğunca yerel olarak depolamak ve çevrimiçi depolama sağlayan bulut benzeri hizmetleri daha az kullanmak.
    - E-) Mobil internet bağlantısı yerine yerel ağ bağlantılarını tercih etmek

    > **Açıklama:** İklim değişikliği konusunda çalışmalar yapan My Climate organizasyonu dijital karbon ayak
    > izini azaltmak için tüm bilgisayar ve internet kullanıcılarına şunları önermektedir:
    > • Video akışını (streaming) azaltmak. Video akışı evrensel veri trafiğinin yüzde 75’ini oluşturmakta,
    > videonun görüntülendiği cihazın üretimi, internet ağ enerji tüketimi, sürece dâhil olan sürücüler
    > ve videonun görüntülenmesi esnasında aracın kendi enerji tüketimi de hesaba katıldığında ne denli
    > bir kaynak ve enerji tüketimi yaşandığı anlaşılmaktadır.
    > • Cihazları daha uzun süre kullanmak.
    > • Kullanılmayan elektronik cihazları doğru şekilde geri dönüşüme kazandırmak.
    > • E-posta kutularını düzenli olarak boşaltarak gereksiz depolama alanı kullanımını engellemek.
    > • Dosyaları mümkün olduğunca yerel olarak depolamak ve çevrimiçi depolama sağlayan bulut benzeri hizmetleri daha az kullanmak.
    > • Mobil internet bağlantısı yerine yerel ağ bağlantılarını tercih etmek (Myclimate, 2022)
    > E-posta kutularını her zaman dolu tutmak. Verileri masa üzerine kayıt etmemek.

    <hr />
1. "Mantıklı bir düzen ve iyi tasarlanmış bir öncelik sıralaması, tasarımın algı etkisini artırır. Görünenden fazla bilgi ve mesajın profesyonel bir dil ve anlaşılır, hazmedilebilir, açıklayıcı bir biçimle görsel olarak tasarlanmasıdır. Başka bir değişle, dar bir alanda, kısa sürede, çok fazla bilgiyi verme işinin başarılmasıdır."<br />Yukarıda sözü edilen tanım aşağıdaki şıklardan hangisini tanımlamaktadır?
    - **Cevap A-) Bibgigrafi**
    - B-) Tipografi
    - C-) Vektörel grafik
    - D-) Raster grafik
    - E-) Dijital baskı

    > **Açıklama:** Mantıklı bir düzen ve iyi tasarlanmış bir öncelik sıralaması, tasarımın algı etkisini artırır. Bilgigrafi, görünenden fazla bilgi ve mesajın profesyonel bir dil ve anlaşılır, hazmedilebilir, açıklayıcı bir biçimle görsel olarak tasarlanmasıdır. İşte bu yüzden, maalesef çoğu kez az bir alanda pek çok bilgi verilmek durumunda kalınır. Başka bir değişle, dar bir alanda, kısa sürede, çok fazla bilgiyi verme işinin başarılmasıdır. Bu yüzden iyi bir bilgigrafi tasarımcısı, bir orkestra şefi gibi, her görsel elemanın ne
    > işe yaradığını bilen ve bunları doğru zamanda, doğru tonda ve ritimde kullanmasını bilen kişidir (Uçar, 2019).
    > Bibgigrafi

    <hr />
## Unite 6
1. Aşağıdakilerden hangisi grafik tasarımda üretim süreçlerini oluşturan aşamalardan biri değildir?
    - A-) Tasarım
    - B-) Uygulama
    - **Cevap C-) Yayınlama**
    - D-) Baskı
    - E-) Dağıtım

    > **Açıklama:** Basılı grafik ürünler için üretim süreci en temel haliyle dört aşamaya ayrılır:
    > • Tasarım
    > • Uygulama
    > • Baskı (Baskı öncesi hazırlık, baskı, baskı sonlandırma ve ciltleme)
    > • Dağıtım

    <hr />
1. Aşağıdakilerden hangisi tasarımın özünü ifade eder?
    - **Cevap A-) Konsept**
    - B-) Selefon
    - C-) Braille Alfabesi
    - D-) Forma
    - E-) Mürekkep

    > **Açıklama:** Konsept: Tema olarak da ifade edilebilen tasarım konsepti; tasarıma yön veren temel duygu, kavram ya da fikir olarak tanımlanabilir. Bir tasarımın konsepti o tasarım probleminin çözümü arkasındaki temel fikirdir ve tasarımın özünü ifade eder.

    <hr />
1. Aşağıdakilerden hangisi tasarım ürününün baskı işleminden sonra ilgili kuruma ulaştırıldığı aşamasıdır?
    - A-) Stratejik planlama
    - **Cevap B-) Dağıtım**
    - C-) Görseller ve metin
    - D-) Baskı
    - E-) Sayfa düzeni

    > **Açıklama:** Dağıtım: Baskı işlemi tamamlanan ürünün teslim edileceği kişi, kurum ya da tesise zarar görmeden ulaştırılması.

    <hr />
1. Aşağıdakilerden hangisi temel tasarım ilkelerinin belirli bir düzende biraraya getirildiği aşamadır?
    - A-) Baskı
    - B-) Baskı öncesi hazırlık
    - **Cevap C-) Sayfa düzeni**
    - D-) Görseller ve Metin
    - E-) Uygulama

    > **Açıklama:** Sayfa düzeni ya da diğer bir deyişle yerleşim, tasarımda kullanılacak olan tüm elemanların tasarım yüzeyinde bir araya getirilmesidir. Kullanılacak görsel elemanların boyutu, konumu ve birbirleriyle
    > olan ilişkileri tasarımın verdiği mesajın algılanma şekli üzerinde büyük etkiye sahiptir. Renk bilgisi, kompozisyon kuralları gibi temel tasarım ilkeleri ışığında tasarlanmış bir sayfa düzeni mesajın alıcıya yani hedef kitleye etkili biçimde ulaşmasını sağlar.

    <hr />
1. Aşağıdakilerden hangisi yüksek baskı tekniklerinden biridir?
    - A-) Tifdruk baskı
    - B-) Ofset baskı
    - C-) Serigrafi
    - **Cevap D-) Tipo baskı**
    - E-) Lazer baskı

    > **Açıklama:** Tipo baskı ve fleksografi teknikleri en yaygın kullanılan yüksek baskı yöntemlerinin başında gelir.

    <hr />
1. Aşağıdakilerden hangisi çukur baskı tekniklerinden biridir?
    - A-) tipo baskı
    - B-) fleksografi
    - **Cevap C-) Tifdruk baskı**
    - D-) Ofset baskı
    - E-) Serigrafi

    > **Açıklama:** Grafik üretim süreçlerinde kullanılan çukur baskı yöntemlerinin başında tifdruk baskı gelir.

    <hr />
1. Aşağıdakilerden hangisi elektriksel yüklemeyi temel alan baskı tekniğidir?
    - A-) Ofset baskı
    - **Cevap B-) Dijital baskı**
    - C-) Flesko baskı
    - D-) Serigrafi
    - E-) Tifdruk baskı

    > **Açıklama:** Dijital baskı sistemleri geleneksel baskı sistemlerinin (ofset baskı, flekso baskı vb.) aksine kalıp, klişe, şablon gibi mürekkebi kâğıda taşıyan ara malzemeye ihtiyaç duymazlar ve mekanik bir süreç yerine elektriksel yüklemeyi esas alırlar.

    <hr />
1. Aşağıdakilerden hangisi baskı sonlandırma aşamasında yer alan işlemlerden biri değildir?
    - A-) Tıraşlama
    - B-) Ciltleme
    - C-) Kabartma
    - D-) Selefon kaplama
    - **Cevap E-) Sayfa düzeni**

    > **Açıklama:** Baskı sonlandırma ifadesi söz konusu basılı materyalin baskısı tamamlandıktan sonra bir tasarıma son dokunuşları sağlayan çok çeşitli işlemleri kapsar. Bu işlemler arasında tıraşlama, ciltleme,
    > selefon kaplama, yaldız baskı uygulama, kabartma gibi çok farklı seçenekler bulunur.

    <hr />
1. Aşağıdakilerden hangisi bir ürünün sürdürülebilir kabul edilmesinde gerekli koşullardan biri değildir?
    - A-) Nihai üründeki her malzemenin geri dönüştürülebilirliğini kontrol etmek,
    - B-) Üretim sürecinde minimum israf ve çevresel etkiyi hedeflemek
    - C-) Sera gazı emisyonlarını azaltmak için taşınabilirliği iyileştirmenin yollarını bulmak
    - **Cevap D-) Tüm tedarik zincirinin ve ilgili şirketlerin çevresel etkisini önemsememek**
    - E-) Tüm malzemelerin nereden temin edildiğini belirleyip, daha iyi alternatiflerin mevcut olup olmadığını tespit etmek

    > **Açıklama:** Bir malzemenin ya da ürünün sürdürülebilir kabul edilebilmesi için sağlanması gereken koşullar şunlardır:
    > • Geri dönüştürülmüş malzemelerin, kirletici içermeyen ya da daha az içeren malzemeler gibi daha az zararlı alternatiflerle değiştirilip değiştirilemeyeceklerini görmek için üretimde kullanılan tüm hammaddeleri değerlendirmek,
    > • Nihai üründeki her malzemenin geri dönüştürülebilirliğini kontrol etmek,
    > • Üretim sürecinde minimum israf ve çevresel etkiyi hedeflemek,
    > • Tüm tedarik zincirinin ve ilgili şirketlerin çevresel etkisini incelemek,
    > • Sera gazı emisyonlarını azaltmak için taşınabilirliği iyileştirmenin yollarını bulmak,
    > • Tüm malzemelerin nereden temin edildiğini belirleyip, daha iyi alternatiflerin mevcut olup olmadığını tespit etmek.

    <hr />
1. Aşağıdakilerden hangisi grafik üretim süreçlerinde meydana gelen sıvı atıklardan biridir?
    - A-) Kırpıntı
    - **Cevap B-) Solvent**
    - C-) Maketler
    - D-) Eskizler
    - E-) Kumaşlar

    > **Açıklama:** Grafik üretim süreçlerinde oluşan sıvı atıkların çoğunluğunu baskı aşamalarında kimyasal maddelerin
    > kullanımına bağlı olarak açığa çıkan atık nemlendirme ve temizleme solüsyonları, birçok farklı amaçla kullanılan solventler, kalıpların banyo kimyasalları ve suları, makine temizliğinde kullanılan temizleyiciler ve mürekkep artıkları oluşturur. Başta solvent olmak üzere hem sağlığa zararlı hem de çevreye zarar verme riski taşıyan bu kimyasal atıkların uygun şekilde ortadan kaldırılması sürdürülebilirlik ve çevre bilinci açısından kritik öneme sahiptir.

    <hr />
1. Para ve tahvil benzeri değerli evrak gibi çok yüksek sayılarda basılacak olan ve kusursuz bir baskı kalitesinin gerektiği baskı tekniği aşağıdakilerden hangisidir?
    - A-) Ofset baskı
    - B-) Tipo baskı
    - **Cevap C-) Tifdruk**
    - D-) Serigrafi
    - E-) Dijital baskı

    > **Açıklama:** Ofset ya da tipo baskı kadar yaygın bir kullanıma sahip olmayan **tifdruk baskı yöntemi çoğunlukla para ve tahvil benzeri** değerli evrak gibi çok yüksek sayılarda basılacak olan, baskılar arasında değişmeyen ve kusursuz bir baskı kalitesinin gerektiği durumlarda tercih edilir.

    <hr />
1. 1\. Ofset baskı<br />2\. Tipo baskı<br />3\. Tifdruk<br />4\. Serigrafi<br />5\. Flekso baskı<br />Yukarıdakilerden hangisi/hangileri grafik üretiminde düz baskının en yaygın kullanılan örneklerindendir?
    - A-) Yalnız I
    - **Cevap B-) Yalnız I ve IV**
    - C-) I, III ve IV
    - D-) II, III ve V
    - E-) III, IV ve V

    > **Açıklama:** Düz baskı teknikleri yüksek ve çukur baskı yöntemleri gibi kalıp üzerinde oluşturulan seviye farkına dayanmazlar. Bir önceki ünite olan Sürdürülebilir Grafik Tasarımında Ölçü ve Formatlar ünitesinin Sürdürülebilir Grafik Tasarımında Format Seçimi başlığında da ele alınan **ofset baskı ve serigrafi teknikleri düz baskının grafik üretiminde en yaygın kullanılan örneklerindendir.** Tipo baskı, flekso ve tifdruk tekniklerinin aksine ofset baskıda kullanılan kalıplar çukur ya da kabartılara sahip değildir ve çıplak gözle bakıldığında düz oldukları söylenebilir.

    <hr />
1. Görme engelli bireylere erişilebilirlik için Braille alfabesiyle yazılmış metinlerin elde edilebildiği baskı sonlandırma işlemi aşağıdakilerden hangisidir?
    - A-) Tıraşlama
    - B-) Selefon kaplama
    - **Cevap C-) Gofre**
    - D-) Lak uygulaması
    - E-) Özel kesim

    > **Açıklama:** **Baskı sonrası tasarıma eklenecek gofre (kabartma)** işlemi amblem ya da ürün/marka ismi gibi bir tasarım elemanına uygulandığında tasarıma dekoratif bir özellik kazandırırken, aynı kabartma işlemi ile **görme engelli bireylere erişilebilirlik için Braille alfabesiyle yazılmış metinler de elde edilebilir.**

    <hr />
1. Ofset baskı mürekkeplerinde çözücü olarak aşağıdakilerden hangisi kullanılır?
    - A-) Etanol
    - B-) Alkol
    - C-) Keton
    - **Cevap D-) Yağ**
    - E-) Su

    > **Açıklama:** Çözücüler mürekkebin içinde tek başına ya da farklı türleriyle birlikte kullanılabilir. Tifdruk baskı yöntemlerinde çözücü olarak etanol, etil glikol, etil asetat; fleksografide setil alkol ve izopropil alkol; **ofset baskı mürekkeplerinde ise çözücü olarak yağ kullanımı görülür.**

    <hr />
1. Aşağıdakilerden hangisi çevresel etkilerle ilişkilendirilen HAP ifadesinin açılımıdır?
    - A-) Sentetik kimyasal çözelti
    - B-) Uçucu organik bileşik
    - C-) Karbon ayak izi
    - D-) Nükleer enerji kaynağı
    - **Cevap E-) Tehlikeli hava kirleticiler**

    > **Açıklama:** Grafik üretim süreçlerinin çevresel etkilerinden olan hava kirliliği söz konusu olduğunda VOC ve HAP bilinmesi gereken iki önemli terim olarak karşımıza çıkar. VOC (volatile organic compound) uçucu organik bileşikler, **HAP ise (hazardous air pollutant) zehirli/tehlikeli hava kirleticiler** anlamına gelmektedir.

    <hr />
1. Aşağıdakilerden hangisi "su ayak izi" kavramını doğru olarak tanımlar?
    - A-) Denizlerdeki kirlilliği ve ekosistemi kimyasal analizlerle ölçen bir ölçüm metodudur.
    - **Cevap B-) Bir tüketici ürün veya hizmetin tüm üretim zinciri boyunca kullanılan tatlı su hacmini (litre veya metreküp olarak) ölçen çevresel bir göstergedir.**
    - C-) Kentlerde ihtiyaç duyulan yüzey ve yeraltı tatlı su kaynaklarının toplam hacmi (litre veya metreküp olarak) için kullanılan bir kavramdır.
    - D-) Ticari malın üretiminde kullanılan toplam yağmur suyunu ifade eder.
    - E-) Nüfus ile ilişkili olarak, kirlilik yükünün bertaraf edilmesi ya da azaltılması için kullanılan tatlı su miktarını ifade eder.

    > **Açıklama:** **Su ayak izi**, bir tüketici ürün veya hizmetinin tüm üretim zinciri boyunca kullanılan tatlı su hacmini (litre veya metreküp olarak) ölçen çevresel bir göstergedir.

    <hr />
1. Aşağıdakilerden hangisi yenilenebilir enerji kaynaklarından biri değildir?
    - **Cevap A-) Doğalgaz**
    - B-) Hidrolik
    - C-) Güneş
    - D-) Rüzgar
    - E-) Jeotermal

    > **Açıklama:** Enerjinin dönüşüm süreçlerinde değişmeden kalabilen, tüketilmesine rağmen azalmayan, yenilenen enerji kaynaklarına yenilenebilir enerji kaynakları adı verilir. **Hidrolik, güneş, rüzgar ve jeotermal gibi enerji kaynakları,** **yenilenebilir enerji kaynaklarıdır.**

    <hr />
1. Aşağıdakilerden hangisi baskı süreçlerinde zehirli hava kirleticilerin oluşturduğu gaz atıkların oluşumunu azaltmak için yapılabilecek uygulamalara doğru bir örnek değildir?
    - A-) Serigrafide kullanılan klorlu solventler yerine su bazlı solventler kullanmak
    - B-) Tolüen, benzin ve asetik asit gibi kimyasallar içeren solventlerin kullanımını azaltmak
    - C-) Bilgisayardan doğruca kalıp üretimine imkan veren CTP teknolojisini kullanmak
    - **Cevap D-) Zorunlu durumlarda, kimyasallar içeren içerikleri havalandırma açısından dış mekanda kullanmak**
    - E-) Solvent ve temizleyicilerin saklama ve depolama koşullarına, havalandırma sistemlerine dikkat etmek

    > **Açıklama:** Baskı süreçlerinde çoğunlukla VOC ve HAP tabir edilen zehirli hava kirleticilerin oluşturduğu gaz atıkların oluşumunu azaltmak için yapılabilecek uygulamalara aşağıdaki örnekler verilebilir: • Serigrafide kullanılan klorlu solventler yerine su bazlı solventler kullanmak, • Tolüen, benzin ve asetik asit gibi kimyasallar içeren solventlerin kullanımını azaltmak, • Ofset nemlendirme suyunda ve temizlik solüsyonlarında düşük uçuculukta içerikler kullanmak, • İzopropil alkol kullanımını azaltmak ya da tümüyle bırakmak için organik solventler yerine bitkisel yağ bazlı temizlik ajanları kullanmak, • Bilgisayardan doğruca kalıp üretimine imkan veren CTP teknolojisini kullanmak, • Su bazlı ve UV kurutmalı vernik kullanmak, • Solvent bazlı yapıştırıcılar yerine düşük solvent içerikli yapıştırıcılar, UV kurutma sistemleri, su bazlı yapıştırıcılar ya da ısıyla yapıştırma sistemleri kullanmak, • Ofset blanket ve merdanelerin temizliği için otomatik temizleme sistemlerine geçmek, • Fleksografide boya sıyırma işlemini kapalı sistemle yapan cihazlar kullanmak, • Solvent ve temizleyicilerin saklama ve depolama koşullarına, havalandırma sistemlerine dikkat etmek.

    <hr />
1. Baskı sonlandırma sürecinde kaplama işleminin genellikle marka, amblem, görsel ya da başlık gibi görsel ilginin toplanması amaçlanan belli bir kısmına uygulanmasına ne ad verilir?
    - A-) Selefon kaplama
    - **Cevap B-) Kısmi lak**
    - C-) Özel bıçak kesimi
    - D-) Yaldız
    - E-) Form

    > **Açıklama:** Selefon ya da benzeri kaplama işlemlerinin temelinde iki ya da daha fazla farklı materyalin birleştirilmesi yatar. Bu işlem her iki materyalin avantajlarını bir arada kullanmaya olanak sağlar (Ambrose ve Harris, 2010). Örneğin bir kağıdın selefon ile kaplanması sayesinde hem kağıdın sunduğu emici baskı yüzeyinin avantajı ve hem de selefonun sağladığı suya karşı koruma ile parlaklık özellikleri bir araya getirilmiş olur. **Kısmi lak ise kaplama işleminin sayfanın tamamına değil genellikle marka, amblem, görsel ya da başlık gibi görsel ilginin toplanması amaçlanan belli bir kısmına uygulanmasıdır.**

    <hr />
1. Grafik üretim sürecinde baskı öncesi atık su oluşumuna neden olan etken aşağıdakilerden hangisidir?
    - A-) Rulo/plaka/battaniye temizleyiciler
    - B-) UV lak
    - C-) Tank suyu
    - D-) Yapıştırıcı ve vernik atıkları
    - **Cevap E-) Banyo kimyasalları ve banyo suyu atıkları**

    > **Açıklama:** **Zehirli veya ayrıştırması zor kimyasalların kullanımı, baskı işlemleri esnasında büyük miktarlarda atık su oluşumuna neden olur.** Çinko, krom, baryum, kurşun gibi maddeler içeren mürekkep kalıntıları, atık nemlendirme ve temizleme solüsyonları, solventler ya da metil etil keton, tolüen, ksilen, izoforon ve etanol gibi kimyasallar içeren atıklar, baskı hazırlığı, baskı ve baskı sonrası da dâhil olmak üzere baskı sürecinin her aşamasında oluşan kimyasal atıkların sadece bazılarıdır. **Baskı öncesi hazırlıkta kalıp banyo kimyasalları ve banyo suyu atıkları oluşurken,** baskı aşamasında mürekkep, solvent, rulo/plaka/battaniye temizleyiciler, vernik, UV lak ve tank suyu atıkları, baskı sonrası aşamalarda ise yapıştırıcı ve vernik atıkları ortaya çıkmaktadır.

    <hr />
## Unite 7
1. Bir ürünü üretim aşamasının tamamlanmasından son kullanıcıya ulaşmasına kadar geçen sürede dağıtım, depolama, sergileme, taşıma gibi işlemler sırasında karşılaşabileceği darbe, su, ısı benzeri etmenlerden korumaya aynı zamanda marka ve ürün hakkında bilgilendirmeye yarayan materyallerin genel adı aşağıdakilerden hangisidir?
    - **Cevap A-) Ambalaj**
    - B-) Malzeme
    - C-) Baskı
    - D-) Tipografi
    - E-) Serigrafi

    > **Açıklama:** Ambalaj, bir ürünü üretim aşamasının tamamlanmasından son kullanıcıya ulaşmasına kadar geçen sürede dağıtım, depolama, sergileme, taşıma gibi işlemler sırasında karşılaşabileceği darbe, su, ısı benzeri etmenlerden korumaya aynı zamanda marka ve ürün hakkında bilgilendirmeye yarayan materyallerin genel adıdır. Ambalaj üretiminde kartondan plastiğe, kumaştan cama kadar çok farklı form ve özelliklere sahip malzemelerden yararlanılabilir. Endüstriyel ürünlerin neredeyse tamamına yakını bir ambalaja ihtiyaç duyar.

    <hr />
1. Konu sürdürülebilirlik olduğunda grafik ürünlerin içinde en problemli olan konu aşağıdakilerden hangisidir?
    - **Cevap A-) Ambalaj**
    - B-) Baskı
    - C-) Serigrafi
    - D-) PNG
    - E-) Litografi

    > **Açıklama:** Konu sürdürülebilirlik olduğunda grafik ürünlerin içinde en problemli konu ambalajdır. Günlük yaşantınızı gözden geçirdiğinizde olağan bir günde ne kadar çok sayıda ambalajlı ürün tükettiğinizi fark etmeniz mümkündür.

    <hr />
1. Aşağıdakilerden hangisi iyi bir ambalajın özellikleri arasında sayılamaz?
    - A-) Bilgilendirme yeterliği
    - B-) Nasıl açıldığı ya da kapandığı
    - C-) Biçim ve ebatları
    - D-) Dayanıklılıklılığı
    - **Cevap E-) Litografisi**

    > **Açıklama:** İyi bir ambalajın bilgilendirme yeterliği, nasıl açıldığı ya da kapandığı, biçim ve ebatları kadar önemli diğer bir kıstas ise şüphesiz ki dayanıklılıktır.

    <hr />
1. Bir ürünle doğrudan temas hâlinde olan ve ürünün kendini kapsayan ambalaja verilen isim aşağıdakilerden hangisidir?
    - **Cevap A-) Birincil ambalaj**
    - B-) İkincil ambalaj
    - C-) Nakliye ambalajı
    - D-) Filmli ambalaj
    - E-) Paletli ambalaj

    > **Açıklama:** Ambalajları kullanım amaçlarına göre sınıflandırmak mümkündür. Bunlardan ilk ikisi olan birincil ve ikincil ambalaj, tüketicilerin daha sık karşılaştıkları ambalajlardır. Satış ambalajı ya da iç ambalaj olarak da adlandırılan birincil ambalaj, bir ürünle doğrudan temas hâlinde olan ve ürünün kendini kapsayan ambalajdır. İkincil ambalaj ya da dış ambalaj olarak bilinen ambalaj ise birincil ambalajla kaplanmış olan birden çok ürünü bir arada tutan ambalajdır. Üçüncül ambalaj veya diğer bir ifade ile nakliye ambalajı ise birincil ya da ikincil ambalajın üreticiyle satıcı arasındaki dağıtım ve depolama gibi süreçlerde zarar görmesini engellemek, taşıma ve depolama kolaylığı sağlamak amacıyla kullanılır

    <hr />
1. Ambalaj sektöründe teneke kutular ve varillerin üretiminde karşımıza çıkan çelik, aşağıdaki hangi bileşenlerin eklenmesiyle demir cevherinden oluşturulur?
    - **Cevap A-) Karbon ve az miktarda diğer alaşım elementleri**
    - B-) Titanyum ve az miktarda diğer alaşım elementleri
    - C-) Bor ve az miktarda diğer alaşım elementleri
    - D-) Oksijen ve az miktarda diğer alaşım elementleri
    - E-) Ahşap ve az miktarda diğer alaşım elementleri

    > **Açıklama:** Ambalaj sektöründe teneke kutular ve varillerin üretiminde karşımıza çıkan çelik, karbon ve az miktarda diğer alaşım elementlerinin eklenmesiyle demir cevherinden oluşturulur. Yerkabuğunun yaklaşık yüzde 5’i (ağırlıkça) demir olduğundan bu element bol miktarda bulunur. Yerkabuğundaki en konsantre demir olan demir cevheri ekonomik açıdan kullanıma uygun olan tek kaynaktır. Çelik üretim sürecindeki ilk adım, demir cevherini yüksek fırında kömür veya kok ve kireçtaşı ile ısıtmaktır. Kömür, oksijeni cevherden uzaklaştırarak serbest demir üretir ve kireçtaşı saf olmayan bileşenlerle reaksiyona girerek ayrıştırılabilen cüruf oluşturur. Yüksek fırında üretilen nispeten saf olmayan demir daha sonra ya açık ocakta ya da bazik oksijen fırınında rafine edilir

    <hr />
1. Ambalaj olarak kullanılan en eski malzemelerden biri olan camın hammaddesi aşağıdakilerden hangisidir?
    - **Cevap A-) Silis kumu**
    - B-) Ambalaj kumu
    - C-) Deniz kumu
    - D-) Çöl kumu
    - E-) Toprak kumu

    > **Açıklama:** Ambalaj olarak kullanılan en eski malzemelerden biri olan camın hammaddesi silis kumudur. Cam yüksek sıcaklıklara maruz bırakılan silis kumu ve çeşitli katkı maddelerinin şekillendirilmesiyle elde edilir.

    <hr />
1. Kağıt için ağartma işlemlerinin bazılarının atık su akıntılarında yayılan hangi maddenin oluşumuna neden olabileceğine dair son keşif kağıt endüstrisi için bir endişe kaynağı haline gelmiştir?
    - **Cevap A-) Dioksin**
    - B-) Karbon
    - C-) Titanyum
    - D-) Bor
    - E-) Etil alkol

    > **Açıklama:** Kağıt için ağartma işlemlerinin bazılarının atık su akıntılarında yayılan dioksin oluşumuna neden olabileceğine dair son keşif kağıt endüstrisi için bir endişe kaynağı haline gelmiştir. Çoğu durumda zehirli maddelerin konsantrasyonları besin zinciri boyunca yükseldikçe artar, balıkları ve diğer deniz organizmalarını insan tüketimi için güvensiz hale getirebilir. Çelik üretimi için cevher ve kömür üretimi hava ve su kirliliğine katkıda bulunduğu bilinen partiküllerin açığa çıkmasına neden olur.

    <hr />
1. Ağartılmış kağıt türlerinden ECF'nin açılımı aşağıdakilerden hangisidir?
    - **Cevap A-) Elementer klorsuz kağıt hamuru**
    - B-) Elementer klorlu kağıt hamuru
    - C-) Elementsiz klorsuz kağıt hamuru
    - D-) Elementsiz klorlu kağıt hamuru
    - E-) Elementersiz klorsuz kağıt hamuru

    > **Açıklama:** ECF (Elemental Chlorine Free Pulp) - Elementer klorsuz kağıt hamuru: Ağartma işleminde elementer klor kademesi kaldırılarak ağartılmış kağıt hamuru.

    <hr />
1. Biyoplastiklerle ilgili aşağıdakilerden hangisi yanlıştır?
    - A-) Nişasta, selüloz, soya proteini ve zirai yan ürünler gibi yenilenebilir ham maddelerden üretilmiş plastiklerdir
    - **Cevap B-) Petrol bazlıdır**
    - C-) Bazı türleri doğada çözünebilir ve/veya komposta dönüşebilir.
    - D-) Tıpkı PET ve diğer plastikler gibi şeffaftır
    - E-) Biyoplastikler hafif ve dayanıklıdır

    > **Açıklama:** Biyoplastikler nişasta, selüloz, soya proteini ve zirai yan ürünler gibi yenilenebilir ham maddelerden üretilmiş plastiklerdir ve petrol bazlı değildirler. Bazı türleri doğada çözünebilir ve/veya komposta dönüşebilir. Tıpkı PET ve diğer plastikler gibi şeffaftırlar. Biyoplastikler hafif ve dayanıklıdır ve diğer plastiklere benzer nem ve gaz bariyeri özelliklerine sahiptir. Genellikle su ve diğer içecekler için şişe yapımında, süt ürünler ve gıda ambalajlarında ya da poşet yapımında kullanılırlar. Diğer petrol bazlı plastiklerden ayrıştırılması için geri dönüşüm altyapısına sahip olmamaları kullanımlarının önündeki en büyük engeldir.

    <hr />
1. Fotobozunur plastiklerin birincil bozunma mekanizması aşağıdakilerden hangisidir?
    - **Cevap A-) Güneş ışığı**
    - B-) Nem
    - C-) Elektrik
    - D-) Soğuk
    - E-) Bakteri

    > **Açıklama:** Fotobozunur plastikler: Birincil bozunma mekanizmasının güneş ışığının etkisiyle olduğu bozunabilir plastiklerdir.

    <hr />
1. Aşağıdakilerden hangisi iyi tasarlanmış bir ambalajın işlevleri arasında yer almaz?
    - A-) Ambalaj yükleme, depolama ve taşıma sırasında ürünü koruyacak şekilde tasarlanmalıdır.
    - B-) Ambalaj koruduğu ürünün içeriği hakkında ilgili tüm taraflara bilgi vermelidir.
    - **Cevap C-) Ambalaj ürün satışında müşteri beğenisini dikkate alacak şekilde tasarlanmalıdır.**
    - D-) Ambalaj içinde bulundurduğu ürünün ve o ürünü üreten markanın müşterileri ile iletişim kurabilmelidir.
    - E-) Ambalaj koruma ve bilgilendirme işlevini yerine getirebilmelidir.

    > **Açıklama:** Ambalaj yükleme, depolama ve taşıma sırasında
    > ürünü koruyacak şekilde ürüne koruyucu ve bil
    > gilendirici bir kaplama sağlama sürecidir. Ayrıca
    > koruduğu ürünün içeriği hakkında ilgili tüm ta
    > raflara (nakliyeci, toptancı, perakendeci, alıcı vb.)
    > faydalı bilgiler sağlamayı amaçlar. İyi bir ambalaj
    > tasarımının işlevleri korumak ve bilgilendirmekle
    > sınırlı değildir. Ambalaj aynı zamanda içinde bu
    > lundurduğu ürünün ve o ürünü üreten markanın
    > müşterileri ya da potansiyel müşterileri ile iletişim
    > kurma aracıdır. Bu özelliğiyle ambalajların ürünün
    > ya da markanın “yüzü” olduğu söylenebilir.

    <hr />
1. Ambalaj tasarımında önemli ölçüde dikkate alınması gereken bilgilendirme kriterinin işlevini yerine getirebilmesi için gerekli unsurlar nelerdir?
    - **Cevap A-) Bilgilerin görünürlüğü, okunurluğu, tasarım hiyerarşisine uygunluğu**
    - B-) Bilgilerin görünürlüğü, dikkat çekici olması, tasarım hiyerarşisine uygunluğu
    - C-) Bilgilerin dikkat çekici olması, okunurluğu, tasarım hiyerarşisine uygunluğu
    - D-) Bilgilerin görünürlüğü, okunurluğu, dikkat çekiciliği
    - E-) Bilgilerin tasarım açısından ilgi çekecek şekilde uygun renk, doku, yazı karakteri ve punto ile verilmesi

    > **Açıklama:** Bu gibi bilgilerin görünürlüğü, okunurluğu, tasarım hiyerarşisine uygunluğu (marka ya da ürünün önüne geçmeyecek kadar geri planda ancak gözden kaçmayacak kadar da görünür oluşu) ambalajın bilgilendirme işlevini yerine getirebilmesi için oldukça önemlidir.

    <hr />
1. Aşağıdakilerden hangisi Etiketlemede söz konusu ürünün markayı temsil etmesi konusunda gerekli kriterler arasında yer almaz?
    - A-) Ürünün ne olduğu bilgisi
    - B-) Nasıl bir ürün olduğuna dair bilgi
    - C-) Hedef kitlenin kimler olduğu tercihi
    - D-) Kurumsal imaja uygun iletişim kurma başarısı
    - **Cevap E-) Müşterinin beğenisini belirleyecek Estetik kaygı gütmesi**

    > **Açıklama:** Etiketlemede en az bilgilendirme kadar önemli diğer bir beklenti ise ambalajın üzerindeki tasarımın söz konusu ürünü ve markayı temsil etmesidir. Bir ambalaj içinde hangi ürünün olduğunu, tüketiciye nasıl bir ürün sunduğunu, hedef kitlesinin kimler olduğunu anlaşılır kılarken aynı zamanda üreticisinin yaratmak istediği kurumsal imaja uygun bir iletişim kurmalıdır. Etiketleme yalnızca estetik bir kaygı değildir.

    <hr />
1. Aşağıdakilerden hangisi ambalajın işlevleri arasında yer almaz?
    - A-) Koruma
    - **Cevap B-) Nakliye**
    - C-) Bir araya getirme
    - D-) Kolaylık sağlama
    - E-) Kapsama

    > **Açıklama:** İyi bir ambalajın hangi özelliklere sahip olması gerektiği sorusunun ardından ambalajın hangi işlevlere sahip olması gerektiği sorusu gelir. Bu soruya verilebilecek çok sayıda cevap olmakla birlikte aşağıda sıralanan şu dört işlev bir ambalajın yerine getirmesi gereken en temel vazifeleri özetleyecektir:
    > 1\. Koruma
    > 2\. Bir araya getirme
    > 3\. Kolaylık sağlama
    > 4\. Kapsama

    <hr />
1. Ambalajlar neye göre sınıflandırılır?
    - A-) Boyut
    - B-) Şekil
    - **Cevap C-) Kullanım amacı**
    - D-) Ürün
    - E-) Satış koşulları

    > **Açıklama:** Ambalajları kullanım amaçlarına göre sınıflandırmak mümkündür.

    <hr />
1. Ambalajlarda kullanılan çok sayıda farklı malzeme çeşidinin olması aşağıdakilerden hangisi ile açıklanamaz?
    - A-) Teknolojik gelişmeler
    - B-) Mevcut doğal kaynakların durumu
    - C-) Tüketici ihtiyaçları
    - **Cevap D-) Dönüştürülebilir materyaller**
    - E-) AR-GE (araştırma – geliştirme) çalışmaları

    > **Açıklama:** Ambalajlarda kullanılan çok sayıda farklı malzeme çeşidi bulunur. Teknolojik gelişmeler, mevcut doğal kaynakların durumu, tüketici ihtiyaçları,AR-GE (araştırma – geliştirme) çalışmaları, bu çeşitliliğin artmasına katkıda bulunur.

    <hr />
1. 2018 yılında yapılan bir araştırmaya göre, dünya çapında oluşan plastik atıkların yaklaşık yüzde kaçı ambalaj atıkları olarak hesaplanmıştır?
    - A-) 32
    - B-) 38
    - **Cevap C-) 46**
    - D-) 53
    - E-) 77

    > **Açıklama:** 2018 yılında yapılan bir araştırmaya göre, dünya çapında oluşan plastik atıkların yaklaşık %46’sını ambalaj atıkları oluşturmaktadır

    <hr />
1. Ambalajlama için kullanılan malzemelerle bu malzemelerin üretim – dağıtım süreçlerinin çevre ve<br />insan sağlığı üzerindeki olumsuz etkileri sıralandığında hangilerinden bahsedilebilir?<br />I-Sınırlı doğal kaynakların sarfiyatı,<br />II- Üretim aşamasında yapılan enerji tüketimi,<br />III- Üretim işlemleri esnasında ortaya çıkan zehirli atıkların oluşturduğu hava ve su kirliliği,<br />IV-Hammaddenin ya da ürünün taşınması için ihtiyaç duyulan enerji ihtiyacı ve<br />V- Ambalajların işlevini tamamlamasıyla oluşan atıkların yarattığı çevre kirliliği
    - A-) I, II, III, IV
    - B-) I, II, IV, V
    - C-) I, III, IV, V
    - D-) II, III, IV, V
    - **Cevap E-) I, II, III, IV. V**

    > **Açıklama:** Ambalajlama için kullanılan malzemelerle bu malzemelerin üretim – dağıtım süreçlerinin çevre ve
    > insan sağlığı üzerindeki olumsuz etkileri; sınırlı doğal kaynakların sarfiyatı, üretim aşamasında yapılan enerji tüketimi, üretim işlemleri esnasında ortaya çıkan zehirli atıkların oluşturduğu hava ve su kirliliği, hammaddenin ya da ürünün taşınması için ihtiyaç duyulan enerji ihtiyacı ve ambalajların işlevini tamamlamasıyla oluşan atıkların yarattığı çevre kirliliği olarak sınıflandırılabilir.

    <hr />
1. Ambalajlama için kullanılan malzemelerle bu<br />malzemelerin üretim – dağıtım süreçlerinin çevre ve<br />insan sağlığı üzerindeki olumsuz etkileri sıralandığında;<br />I- sınırlı doğal kaynakların sarfiyatı,<br />II- üretim aşamasında yapılan enerji tüketimi,<br />III- üretim işlemleri esnasında ortaya çıkan zehirli atıkların oluşturduğu hava ve su kirliliği,<br />IV-hammaddenin ya da ürünün taşınması için ihtiyaç duyulan enerji ihtiyacı,<br />V- ambalajların işlevini tamamlamasıyla oluşan atıkların yarattığı çevre kirliliği olarak sınıflandırılabilir<br />Yukarıdakilerden hangileri doğru ifadeyi oluşturur?
    - A-) I, III, IV, V
    - B-) II, III, IV,V
    - C-) I, IV, V
    - **Cevap D-) I, II, III, IV, V**
    - E-) I, III, V

    > **Açıklama:** Ambalajlama için kullanılan malzemelerle bu
    > malzemelerin üretim – dağıtım süreçlerinin çevre ve
    > insan sağlığı üzerindeki olumsuz etkileri; sınırlı do
    > ğal kaynakların sarfiyatı, üretim aşamasında yapılan
    > enerji tüketimi, üretim işlemleri esnasında ortaya çı
    > kan zehirli atıkların oluşturduğu hava ve su kirliliği,
    > hammaddenin ya da ürünün taşınması için ihtiyaç
    > duyulan enerji ihtiyacı ve ambalajların işlevini ta
    > mamlamasıyla oluşan atıkların yarattığı çevre kirli
    > liği olarak sınıflandırılabilir.

    <hr />
1. Bir ambalajın yerine getirmesi gereken en temel işlevler hangileridir?
    - A-) Tanıtım, koruma, depolama, görsel çekicilik
    - **Cevap B-) Koruma, bir araya getirme, kolaylık sağlama, kapsama**
    - C-) Koruma, taşıma, dayanıklılık, estetik
    - D-) Satışı artırma, pazarlama, depolama, kullanım kolaylığı
    - E-) Stoklama, içeriği koruma, dayanıklılık, marka bilinirliği

    > **Açıklama:** Ambalajın İşlevleri
    > İyi bir ambalajın hangi özelliklere sahip olması gerektiği sorusunun ardından ambalajın hangi işlevlere sahip olması gerektiği sorusu gelir. Bu soruya verilebilecek çok sayıda cevap olmakla birlikte aşağıda sıralanan şu dört işlev bir ambalajın yerine getirmesi gereken en temel vazifeleri özetleyecektir:
    > 1\. Koruma
    > 2\. Bir araya getirme
    > 3\. Kolaylık sağlama
    > 4\. Kapsama

    <hr />
1. Ambalaj tasarımında sürdürülebilirliğe en çok etki eden özelliklerin başında yer alan unsur nedir?
    - A-) Üretim yöntemi
    - B-) Renk
    - C-) Ölçü
    - **Cevap D-) Malzeme**
    - E-) Format

    > **Açıklama:** Ambalaj da dâhil olmak üzere tüm basılı grafik
    > ürünlerin sürdürülebilirliğinde rol oynayan pek çok
    > unsur bulunmaktadır. Üretim yöntemi, mürekkep,
    > format ya da ölçü gibi nitelikler basılı grafik ürün
    > lerin ne denli sürdürülebilir olduğunu belirlemede
    > etkilidir. Ancak konu ambalaj tasarımı olduğunda
    > sürdürülebilirliğe en çok etki eden özelliklerin başın
    > da malzeme gelir.

    <hr />
1. Organik ya da inorganik yeni kaynak ihtiyacını ve üretimde harcanan enerji miktarını<br />düşürmenin, atıkların oluşturduğu çevresel ve sağlık tehdidini azaltmanın en basit yolu nedir?
    - **Cevap A-) İhtiyacı azaltmak ve atık oluşumunu en başta önlemek**
    - B-) Sürdürülebilir tasarımları artırmak
    - C-) Geri dönüşüm malzemeler kullanmak
    - D-) Enerji ve iş gücünü düşürmek
    - E-) Daha basit ambalaj tasarımları yapmak

    > **Açıklama:** Kaynağı ve üretim yöntemi ne olursa olsun bütün ürünler üretimi, taşıması, atığa dönüşmesi ya da ihtiyaç duyduğu geri dönüşüm işlemleri gereği az ya da çok çevresel etkiye sahiptir, iş gücü ve zaman gerektirir. Organik ya da inorganik yeni kaynak ihtiyacını ve üretimde harcanan enerji miktarını düşürmenin, atıkların oluşturduğu çevresel ve sağlık tehdidini azaltmanın en basit yolu bu ihtiyacı azaltmak ve atık oluşumunu en başta önlemektir.

    <hr />
1. Aşağıdakilerden hangisi, ambalajların çoğunlukla muhafaza ettikleri ürünün ilk kullanımında ya da ürünün tüketilmesiyle işlevini yitirerek atığa dönüşmesini engelleyen etkili yollardan birisidir?
    - A-) Birden fazla kullanıma yönelik tasarımlar
    - B-) Geri dönüştürülebilir malzemelerden yapılan tasarımlar
    - C-) Kağıt ve türevim ürünlerden yapılan tasarımlar
    - D-) Sürdürülebilir malzemelerden üretilen tasarımlar
    - **Cevap E-) Ürünü korumanın dışında ilave bir işlev kazandırmaya yarayan tasarımlar**

    > **Açıklama:** Basılı grafik ürünlerin kimileri saklamak, biriktirmek, yeniden kullanmak gibi amaçlarla uzun bir kullanım ömrüne sahip olsalar da ambalajlar çoğunlukla muhafaza ettikleri ürünün ilk kullanımında ya da ürünün tüketilmesiyle işlevini yitirerek atığa dönüşürler. Bunu engellemenin etkili yollarından biri de ambalaja ürünü korumanın dışında ilave bir işlev kazandırmaya yarayan tasarım fikirleridir.

    <hr />
1. Bir ambalajın yerine getirmesi gereken en temel işlevler hangileridir?
    - A-) Tanıtım, koruma, depolama, görsel çekicilik
    - **Cevap B-) Koruma, bir araya getirme, kolaylık sağlama, kapsama**
    - C-) Koruma, taşıma, dayanıklılık, estetik
    - D-) Satışı artırma, pazarlama, depolama, kullanım kolaylığı
    - E-) Stoklama, içeriği koruma, dayanıklılık, marka bilinirliği

    > **Açıklama:** Ambalajın İşlevleri
    > İyi bir ambalajın hangi özelliklere sahip olması gerektiği sorusunun ardından ambalajın hangi işlevlere sahip olması gerektiği sorusu gelir. Bu soruya verilebilecek .ok sayıda cevap olmakla birlikte aşağıda sıralanan şu dört işlev bir ambalajın yerine getirmesi gereken en temel vazifeleri özetleyecektir:
    > 1\. Koruma
    > 2\. Bir araya getirme
    > 3\. Kolaylık sağlama
    > 4\. Kapsama

    <hr />
## Unite 8
1. Aşağıdakilerden hangisi sürdürülebilir grafik tasarımcısının kağıt tüketimindeki rolü olamaz?
    - **Cevap A-) Tasarımlarını çoğunlukla kağıt üzerine çıktı alır.**
    - B-) Hangi ürünün kağıt üzerinde hangisinin dijital ortamda iş göreceğini bilir.
    - C-) Kağıt kullanımına yönelik müşterisine uyarılar yapar.
    - D-) Ölçüleri doğru kullanarak tasarımını kağıttan fire vermeyecek şekilde oluşturabilir.
    - E-) Kağıt üreticilerine ve matbaalara baskı yaparak çevreye daha az zarar verecek ürünlerin ön plana çıkarılmasını sağlayabilir.

    > **Açıklama:** Kâğıdı hem tüketen hem de tüketimini yönlendiren grafik tasarımcının konu hakkında bilinç sahibi
    > olması, özellikle son üründe ciddi farklar yaratabilir. Sürdürülebilir dünya görüşüne sahip olan grafik tasarımcı; hangi ürünün kağıt üzerinde hangisinin dijital ortamda iş göreceğini bilir, seçtiği kağıt için daha çevre dostu alternatifleri araştırabilir, ölçüleri doğru kullanarak tasarımını kağıttan fire vermeyecek şekilde oluşturabilir, kağıt kullanımına yönelik müşterisine yapacağı uyarılarla firmanın imajını destekleyebilir, kağıt üreticileri ve matbaalar gibi ortaklaşa çalıştığı iş sektörlerine baskı yaparak çevreye daha az zarar verecek ürünlerin ön plana çıkarılmasını sağlayabilir.

    <hr />
1. Komünist ideolojiyi savunan Grapus grubu ne zaman hangi ülkede kurulmuştur?
    - A-) 1980- Viyana
    - **Cevap B-) 1970- Fransa**
    - C-) 1975- İtalya
    - D-) 1988- Avusturya
    - E-) 1990- Almanya

    > **Açıklama:** 1970 ile 1991 yılları arasında varlık gösteren Grapus grubu, tasarımın mükemmelliğini toplumsal vicdanla birleştirmeye çalışan bir grafik sanatçıları kolektifiydi. Grapus, 1970 yılında komünist ideolojiyi savunan Pierre Bernard, François Miehe ve Gérard Paris-Clavel tarafından Fransa’da kurulmuştur.

    <hr />
1. Aşağıdaki grafik tasarımcılarından hangisi sosyal sorumluluk ve etik konularından önemli bir isimdir?
    - A-) Pierre Bernard
    - B-) François Miehe
    - C-) Gérard Paris-Clavel
    - **Cevap D-) Tibor Kalman**
    - E-) Steve Cutts

    > **Açıklama:** Tibor Kalman, özellikle grafik tasarıma yaklaşımı ve sosyal afişleriyle anılması gereken isimlerden biridir. Budapeşte’de doğan tasarımcı; 1956’da Amerika Birleşik Devletleri’ne taşınmış ve 1979’da M&Co isimli kendi tasarım stüdyosunu kurmuştur. Tibor Kalman, grafik tasarıma bakış açısı ile tasarımcının sosyal sorumluluğu ve grafik tasarımda etik konularında yönlendirici bir isim olmuştur.

    <hr />
1. Aşağıdakilerden hangisi tasarımcının müşterilere karşı sorumluluğundan biri değildir?
    - A-) Tasarımcı, müşterinin iş ve tasarım standartları hakkında bilgi sahibi olacak ve mesleki sorumluluk sınırları dâhilinde müşterinin çıkarlarına en uygun şekilde hareket edecektir.
    - B-) Tasarımcı, ilgili müşterilerin veya işverenlerin anlaşması olmadan çıkar çatışması yaratan görevlerde aynı anda çalışmayacaktır.
    - **Cevap C-) Tasarımcı, etik olmayan teşviklerle doğrudan veya dolaylı olarak başka bir tasarımcının yerini almaya veya onunla rekabet etmeye çalışmamalıdır.**
    - D-) Tasarımcı, bir projenin tamamlanmasından önce devam eden tüm çalışmaları ve müşteri hakkındaki bilgileri tutacak ve müşterinin rızası olmadan bu bilgileri hiçbir şekilde ifşa etmeyecektir.
    - E-) Müşteri veya işverenden gelen, tasarımcının etik standartlarının ihlalini içeren talimatlar, tasarımcı tarafından düzeltilmelidir.

    > **Açıklama:** **Tasarımcının Müşterilere Karşı Sorumluluğu Şunlardır:**
    > a. Tasarımc**ı, müşterinin iş ve tasarım standa**rtları hakkında bilgi sahibi olacak ve mesleki sorumluluk sınırları dâhilinde müşterinin çıkarlarına en uygun şekilde hareket edecektir.
    > b. T**asarımcı, ilgili müşterileri**n veya **işverenlerin** anlaşması olmadan çıkar çatışması yaratan görevlerde aynı anda çalışmayacaktır.
    > c. **Tasarımcı, bir projenin t**amamlanmasından önce devam eden tüm çalışmaları ve müşteri hakkındaki bilgileri tutacak ve müşterinin rızası olmadan bu bilgileri hiçbir şekilde ifşa etmeyecektir. Tüm personelin buna göre hareket etmesini sağlamak, tasarımcının sorumluluğundadır.
    > d. **Müşteri veya** işverenden gelen, tasarımcının etik standartlarının ihlalini içeren talimatlar, tasarımcı tarafından düzeltilmelidir veya reddetmelidir.
    > **Tasarımcının Diğer Tasarımcılara Karşı Sorumluluğu:**
    > **c. Tasarımcı, etik olmayan teşviklerle doğrudan veya dolaylı olarak başka bir tasarımcının yerini almaya veya onunla rekabet etmeye çalışmamalıdır.**

    <hr />
1. Aşağıdakilerden hangisi tasarımcının halka karşı sorumluluğundan biridir?
    - A-) Tasarımcı, ilgili müşterilerin veya işverenlerin anlaşması olmadan çıkar çatışması yaratan görevlerde aynı anda çalışmayacaktır
    - **Cevap B-) Tasarımcı, her durumda ve her zaman gerçeği iletmelidir; çalışmaları yanlış iddialarda bulunmamalı veya bilerek yanlış bilgi vermemelidir.**
    - C-) Tasarımcı, bir projenin tamamlanmasından önce devam eden tüm çalışmaları ve müşteri hakkındaki bilgileri tutacak ve müşterinin rızası olmadan bu bilgileri hiçbir şekilde ifşa etmeyecektir.
    - D-) Müşteri veya işverenden gelen, tasarımcının etik standartlarının ihlalini içeren talimatlar, tasarımcı tarafından düzeltilmelidir.
    - E-) Tasarımcı, müşterinin iş ve tasarım standartları hakkında bilgi sahibi olacak ve mesleki sorumluluk sınırları dâhilinde müşterinin çıkarlarına en uygun şekilde hareket edecektir.

    > **Açıklama:** Tasarımcının Halka Karşı Sorumluluğu:
    > a. Tasarımcı, halka zarar verecek projelerden kaçınmalıdır.
    > b. Tasarımcı, her durumda ve her zaman gerçeği iletmelidir; çalışmaları yanlış iddialarda bulunmamalı veya bilerek yanlış bilgi vermemelidir. Profesyonel tasarımcı, her
    > türlü iletişim tasarımında mesajları açık bir şekilde temsil etmeli ve yanlış, yanıltıcı ve
    > aldatıcı tanıtımlardan kaçınmalıdır.
    > c. Tasarımcı, tüm izleyicilerin onuruna ve bireysel farklılıklara saygı göstermeli, insanları veya insan gruplarını olumsuz, insanlıktan çıkarıcı bir şekilde tasvir etmekten veya klişeleştirmekten kaçınmalıdır. Profesyonel bir tasarımcı, kültürel değerlere ve inançlara duyarlı olmaya çalışmalı ve karşılıklı anlayışı teşvik eden ve cesaretlendiren adil ve dengeli iletişim tasarımı geliştirmelidir.

    <hr />
1. Aşağıdakilerden hangisi tasarımcının topluma ve çevreye karşı sorumluluğundan biri değildir?
    - A-) Tasarımcı, ırk, cinsiyet, yaş, din, ulusal köken, cinsel yönelim veya engellilik temelinde ayrımcılık yapmayı reddetmeli veya bu ayrımcılığı desteklememelidir.
    - B-) Tasarımcı, imalatçılar, tedarikçiler veya üstlenici tarafından sunulan ve müşterisinin, toplumun veya çevrenin çıkarlarına zarar veren mal veya hizmetleri bilerek kullanamaz.
    - C-) Tasarımcı, zamanının yüzde beşini, topluma hizmet eden ve insan deneyimini geliştiren, kamu yararı gözeten projelere katkıda bulunmaya ayırmalıdır.
    - D-) Tasarımcı, ifade özgürlüğü, toplanma özgürlüğü ve açık fikir pazarına erişim ilkelerini anlamaya ve desteklemeye çalışmalı ve buna göre hareket etmelidir.
    - **Cevap E-) Tasarımcı, başka bir tasarımcının çalışmasını eleştirirken objektif ve dengeli olmalı ve bir tasarımcı meslektaşının eserini veya itibarını karalamamalıdır.**

    > **Açıklama:** Tasarımcının Topluma ve Çevreye Karşı Sorumluluğu:
    > Tasarımcı, uygulama ya da tasarım eğitiminde, içinde yaşadığı toplulukların sağlık, güvenlik ya da mahremiyetini kasıtlı ve pervasız bir şekilde göz ardı eden herhangi bir şeyi bilerek ve isteyerek yapmayacaktır. Profesyonel tasarımcı; insanların görsel tasvirinde, doğal kaynakların tüketiminde,
    > hayvanların ve çevrenin korunmasında sorumlu bir rol üstlenmelidir.
    > b. Tasarımcı, zamanının yüzde beşini, topluma hizmet eden ve insan deneyimini geliştiren, kamu yararı gözeten projelere katkıda bulunmaya ayırmalıdır.
    > c. Tasarımcı, yaptığı işin çevresel, ekonomik, sosyal ve kültürel sonuçlarını dikkate almalı ve olumsuz etkileri en aza indirmelidir.
    > d. Tasarımcı, başka bir kişinin veya grubun insan haklarının veya mülkiyet haklarının ihlalini içeren talimatları, söz konusu diğer kişi veya grubun izni olmadan bilerek kabul etmeyecek veya bu tür ihlalleri içeren herhangi bir bilinçli eylemde bulunmayacaktır.
    > e. Tasarımcı, imalatçılar, tedarikçiler veya üstlenici tarafından sunulan ve müşterisinin, toplumun veya çevrenin çıkarlarına zarar veren mal veya hizmetleri bilerek kullanamaz.
    > f. Tasarımcı, ırk, cinsiyet, yaş, din, ulusal köken, cinsel yönelim veya engellilik temelinde ayrımcılık yapmayı reddetmeli veya bu ayrımcılığı desteklememelidir.
    > g. Tasarımcı, ifade özgürlüğü, toplanma özgürlüğü ve açık fikir pazarına erişim ilkelerini anlamaya ve desteklemeye çalışmalı ve buna göre hareket etmelidir.

    <hr />
1. Aşağıdakilerden hangisi sürdürülebilir olmayan bir firmanın çevre dostu olduğunu iddia eden uygulamalarına verilen bir isimdir?
    - A-) İntihal
    - **Cevap B-) Yeşil badana**
    - C-) Brief
    - D-) E-atık
    - E-) Yersiz iddia

    > **Açıklama:** Yeşil Badana (Greenwashing): Sürdürülebilir olmayan firma, ürün ya da hizmetin çevre dostu olduğunu iddia ya da ima eden, tüketiciyi yanıltmaya yönelik çeşitli uygulamalar yeşil badana olarak adlandırılır.

    <hr />
1. Aşağıdakilerden hangisi grafik tasarımcı tarafından yapılacak işi tanımlar?
    - **Cevap A-) Brief**
    - B-) İntihal
    - C-) Yeşil badana
    - D-) E-atık
    - E-) Kağıt gramajı

    > **Açıklama:** Brief, kelime olarak kısa, öz bilgi demektir. Grafik tasarımcı açısından brief, yapılacak olan işin tanımıdır. Grafik tasarımcı brief ile çalışır. Grafik tasarım bir iletişim mesajını görsel olarak biçimlendirmektir.

    <hr />
1. Aşağıdakilerden hangisi sürdürülebilir grafik tasarımında ölçü ve format konusunda dikkate alınması gereken önlemlerden biri değildir?
    - A-) Kağıdın iki yüzünü kullanma
    - B-) Kelime ve satır esaslarını daraltma
    - C-) Yazı ölçüsünü küçültme
    - **Cevap D-) Kenar boşlukları genişletme**
    - E-) Dokümanın ölçünü küçültme

    > **Açıklama:** Ölçü ve format konusunda; kâğıdın iki yüzünü de kullanma, oluşturulacak dokümanın ölçüsünü küçültme, okunurluk ve estetiği koruyarak kelime ve satır esaslarını daraltma, kenar boşluklarını azaltma, yazı ölçüsünü küçültmek gibi kâğıt miktarını azaltacak öneriler geliştirilebilir ve mümkünse uygulanabilir.

    <hr />
1. Aşağıdakilerden hangisi kağıdın kalınlığını tanımlar?
    - A-) İntihal
    - B-) Yeşil badana
    - **Cevap C-) Kağıt gramajı**
    - D-) E-atık
    - E-) Brief

    > **Açıklama:** Kâğıt gramajı: Kâğıdın bir metrekaresinin ağırlığını ifade eder. Kâğıdın kalınlığını tanımlamak için kullanılır.

    <hr />
1. 1999 yılında, “tasarım mesleğinin ahlaki pusulası ve en ateşli provokatörü” olarak AIGA madalyası ile onurlandırılan tasarımcı kimdir?
    - A-) Pierre Bernard
    - B-) Ken Garland
    - **Cevap C-) Tibor Kalman**
    - D-) David Bailey
    - E-) Jeremy Pemberton

    > **Açıklama:** Tibor Kalman, 1999 yılında, “tasarım mesleğinin ahlaki pusulası ve en ateşli provokatörü” olarak AIGA madalyası ile onurlandırılmıştır.

    <hr />
1. Aşağıdakilerden hangisi grafik tasarımda reklam karşıtlığıyla en fazla ön plana çıkan yayındır?
    - A-) Colors Dergisi
    - **Cevap B-) Adbusters Dergisi**
    - C-) Marketingprofs Blog
    - D-) Watchmen Dergisi
    - E-) Dribbble.com

    > **Açıklama:** Grafik tasarımda reklam karşıtlığıyla en fazla ön plana çıkan yayın Adbusters Dergisidir. 1989 yılında Kalle Lasn ve Bill Schmalz tarafından Kanada’da kurulmuş kâr amacı gütmeyen bir organizasyondur.

    <hr />
1. Aşağıdakilerden hangisi 1914 yılında grafik tasarım özelinde ABD’de kurulan ve şu anda bütün tasarım alanlarına hitap eden önemli bir organizasyondur?
    - A-) UNESCO
    - B-) WWF
    - C-) BBC
    - **Cevap D-) AIGA**
    - E-) AGFA

    > **Açıklama:** AIGA (American Institute of Graphic Arts), 1914 yılında grafik tasarım özelinde ABD’de kurulmuş, şu anda bütün dünyaya ve bütün tasarım alanlarına hitap eden önemli bir organizasyondur.

    <hr />
1. Mesleki Uygulama Standartlarına göre; “İş olanaklarını takip eden tasarımcılar, adil ve açık rekabeti desteklemelidir” maddesi hangi başlık altında yer almaktadır?
    - A-) Tasarımcının Müşterilere Karşı Sorumluluğu
    - B-) Ücretler
    - C-) Tanıtım
    - D-) Sahiplik
    - **Cevap E-) Tasarımcının Diğer Tasarımcılara Karşı Sorumluluğu**

    > **Açıklama:** AIGA’nın mesleki standartları:
    > Tasarımcının Diğer Tasarımcılara Karşı Sorumluluğu:
    > **a. İş olanaklarını takip eden tasarımcılar, adil ve açık rekabeti desteklemelidir.** b. Profesyonel tasarımcı, başka bir tasarımcının üzerinde çalıştığı veya çalışmakta olduğu herhangi bir profesyonel görevi bilerek kabul etmeyecektir. c. Tasarımcı, etik olmayan teşviklerle doğrudan veya dolaylı olarak başka bir tasarımcının yerini almaya veya onunla rekabet etmeye çalışmamalıdır. d. Tasarımcı, başka bir tasarımcının çalışmasını eleştirirken objektif ve dengeli olmalı ve bir tasarımcı meslektaşının eserini veya itibarını karalamamalıdır. e. Tasarımcı, başka bir kişinin mülkiyet haklarının izinsiz olarak ihlal edilmesini içerentalimatları kabul etmeyecek veya bu tür ihlalleri içeren çalışmalar içinde bilinçli olarak yer almayacaktır. f. Kendi ülkesinden başka bir ülkede çalışan profesyonel bir tasarımcı, ilgili ulusal toplumun davranış kurallarına uyacaktır.
    > Mesleki Uygulama Standartları; Tasarımcının Diğer Tasarımcılara Karşı Sorumluluğu: a. İş olanaklarını takip eden tasarımcılar, adil ve açık rekabeti desteklemelidir.

    <hr />
1. Grafik tasarımda özgün bir çalışma yapmak yerine önceden yapılan kendi çalışmalarımızdan aynen almak hangi eylem altında yer alır?
    - **Cevap A-) İntihal**
    - B-) İlham
    - C-) Esinlenme
    - D-) Devşirme
    - E-) Düzenleme

    > **Açıklama:** İntihal (TDK: aşırma), bir kişinin eserinde başka kişilerin ifade, buluş veya düşüncelerini kaynak göstermeksizin kendisine aitmiş gibi kullanmasıdır. Kınık (2015), tasarımda yapılan intihaller sınıflandırması f maddesi: “Özgün bir çalışma yapmak yerine önceden yapılan kendi çalışmalarımızdan aynen almak”tır.

    <hr />
1. Grafik tasarımcı açısından aşağıdakilerden hangisi yapılacak olan işi tanımlayan kavramdır?
    - A-) Bilboard
    - **Cevap B-) Brief**
    - C-) Eskiz
    - D-) Prototip
    - E-) Tasarım

    > **Açıklama:** Brief, kelime olarak kısa, öz bilgi demektir. Grafik tasarımcı açısından brief, yapılacak olan işin tanımıdır.

    <hr />
1. Aşağıdakilerden hangisi kâğıdın bir metrekaresinin ağırlığını ifade eder?
    - A-) Selüloz
    - B-) Ton
    - C-) Kilogram
    - **Cevap D-) Gramaj**
    - E-) Santimetre

    > **Açıklama:** Kâğıt gramajı: Kâğıdın bir metrekaresinin ağırlığını ifade eder.

    <hr />
1. Aşağıdakilerden hangisi belli bir sorunu çözmeyi hedefleyen, veri ve bilgiye dayalı, ancak sanat ve hayal gücünün de dahil olduğu, planlı bir üretim sürecini tanımlar?
    - A-) Resim
    - B-) Baskı
    - C-) Proje
    - D-) Ambalaj
    - **Cevap E-) Tasarım**

    > **Açıklama:** Tasarım, belli bir sorunu çözmeyi hedefleyen, veri ve bilgiye dayalı, ancak sanat ve hayal gücünün de dahil olduğu, planlı bir üretim sürecidir.

    <hr />
1. Steve Cutts’un 2012 yılında görselliğin gücünü gözler önüne serer nitelikteki, söz içermeyen kısa animasyon filminin adı nedir?
    - A-) Human
    - B-) Earth
    - **Cevap C-) Man**
    - D-) Woman
    - E-) Child

    > **Açıklama:** [https://youtu.be/WfGMYdalClU](https://youtu.be/WfGMYdalClU)
    > Steve Cutts’un 2012 yılında Flash & After Effects programlarıyla ürettiği, “Man” isimli söz içermeyen kısa animasyon filmi, görselliğin gücünü gözler önüne serer niteliktedir.

    <hr />
1. Reklam aracı olarak grafik tasarım, ……………….. ve genel olarak ……………….. yöneltir.<br />Yukarıdaki cümlede boş bırakılan yerleri aşağıdakilerden hangileri doğru şekilde tamamlar?
    - A-) Bilgilendirir - eğlenceye
    - **Cevap B-) Yönlendirir - tüketime**
    - C-) Görselleştirir - reklama
    - D-) Beğenilir - televizyona
    - E-) Eğitir - bilgiye

    > **Açıklama:** Reklam aracı olarak grafik tasarım, yönlendirir ve genel olarak tüketime yöneltir.

    <hr />
